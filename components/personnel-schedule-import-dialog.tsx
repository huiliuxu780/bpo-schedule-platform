...
import {
  type PersonnelScheduleImportDialogStepKey,
...
export function PersonnelScheduleImportDialog({
  dialog,
  templateError,
  action,
}: {
  ...
  const [activeStep, setActiveStep] =
    React.useState<PersonnelScheduleImportDialogStepKey>(
  ...
  return (
    <Dialog open onOpenChange={closeDialog}>
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-hidden sm:max-w-5xl">
        <DialogHeader>
          <DialogTitle>排班导入</DialogTitle>
          <DialogDescription>
            从排班版本页发起导入；行明细、失败修正和应用处理进入批次详情。
          </DialogDescription>
        </DialogHeader>

        <div className="grid gap-4 overflow-y-auto pr-1">
          <PersonnelScheduleImportStepHeader
            steps={dialog.steps}
          ...
          <form ref={formRef} action={action} className="grid gap-4">
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="grid gap-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    先下载模板，按日期、人员、职场、供应商、技能、班次和时间补齐后上传。
                  </p>
                </div>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <Field label="批次号">
                  <Input name="batch_id" placeholder="SCH-20260612-001" required />
                </Field>
                <Field label="文件名">
                  <Input name="file_name" placeholder="personnel-schedule.csv" />
                </Field>
                <Field label="上传人">
            ...
            >
              ...
              <PersonnelScheduleImportResult result={dialog.result} />
            </section>
...
function PersonnelScheduleImportStepHeader({
  steps,
  activeStep,
}: {
  steps: PersonnelScheduleImportDialogSummary["steps"]
  activeStep: PersonnelScheduleImportDialogStepKey
}) {
  return (
...
function MappingModeHeader({
  ...
  mode: PersonnelScheduleImportDialogSummary["mappingModes"][number]
}) {
  return (
    <div className="grid gap-1">
...
function PersonnelScheduleImportResult({
  result,
}: {
  result: PersonnelScheduleImportDialogSummary["result"]
}) {
  if (!result) {
    return (
