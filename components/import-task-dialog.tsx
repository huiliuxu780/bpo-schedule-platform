...
import {
  type ImportTaskDialogSummary,
  type ImportTaskStepKey,
} from "@/lib/import-task-model"
...
// 统一导入向导：参数化三步（上传文件 → 字段映射 → 导入结果）。
// 四种变体（需求预测 / 人员排班 / 实际日志 / 主数据）共用同一交互，
// 差异（标题、提示、字段占位、默认映射、模板下载）全部来自 summary。
export function ImportTaskDialog({
  dialog,
  templateError,
  action,
}: {
  dialog: ImportTaskDialogSummary
  templateError: string | null
  action: (formData: FormData) => Promise<void>
}) {
  const router = useRouter()
  const formRef = React.useRef<HTMLFormElement>(null)
  const [activeStep, setActiveStep] = React.useState<ImportTaskStepKey>(
    ...
    <Dialog open onOpenChange={closeDialog}>
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-hidden sm:max-w-5xl">
        ...
        <div className="grid gap-4 overflow-y-auto pr-1">
          <ImportTaskStepHeader steps={dialog.steps} activeStep={activeStep} />
          ...
          <form ref={formRef} action={action} className="grid gap-4">
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="grid gap-1">
                  ...
                  <p className="text-sm text-muted-foreground">{dialog.uploadHint}</p>
                </div>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <Field label="批次号">
                  <Input
                    name="batch_id"
                    placeholder={dialog.batchIdPlaceholder}
                    required
                  />
                </Field>
                <Field label="文件名">
                  <Input name="file_name" placeholder={dialog.fileNamePlaceholder} />
                </Field>
                <Field label="上传人">
                  <Input
                    name="uploaded_by"
                    defaultValue={dialog.uploaderDefault}
                    required
                  />
                </Field>
                <Field label="CSV 文件">
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                ...
                <div className="grid gap-3 rounded-md bg-muted/30 p-3">
                  ...
                  <Field label="字段映射 JSON">
                    <textarea
                      name="field_mapping"
                      defaultValue={JSON.stringify(dialog.defaultFieldMapping, null, 2)}
            ...
            >
              ...
              <ImportTaskResult result={dialog.result} />
            </section>
...
function ImportTaskStepHeader({
  steps,
  activeStep,
}: {
  steps: ImportTaskDialogSummary["steps"]
  activeStep: ImportTaskStepKey
}) {
  return (
...
function MappingModeHeader({
  ...
  mode: ImportTaskDialogSummary["mappingModes"][number]
}) {
  return (
    <div className="grid gap-1">
...
function ImportTaskResult({
  ...
  result: ImportTaskDialogSummary["result"]
}) {
  if (!result) {
    return (
