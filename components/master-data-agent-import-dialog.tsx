...
import {
  type MasterDataAgentImportDialogStep,
...
type AgentImportStepKey = MasterDataAgentImportDialogStep["key"]
...
export function AgentImportDialog({
  ...
  const [activeStep, setActiveStep] = React.useState<AgentImportStepKey>(
  ...
  return (
    <Dialog open onOpenChange={closeDialog}>
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-hidden sm:max-w-5xl">
        <DialogHeader>
          <DialogTitle>客服人员批量导入</DialogTitle>
          <DialogDescription>
...
function AgentImportStepHeader({
  steps,
  activeStep,
}: {
  steps: MasterDataAgentImportDialogStep[]
  activeStep: AgentImportStepKey
}) {
  return (
