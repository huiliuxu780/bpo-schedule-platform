...
const SIDEBAR_STORAGE_KEY = "bpo-sidebar-open"
const SIDEBAR_KEYBOARD_SHORTCUT = "b"

let inMemorySidebarOpen: boolean | null = null

function readSidebarOpenPreference(defaultOpen: boolean) {
  ...
  try {
    ...
    // Continue to cookie and in-memory fallbacks below.
  }

  const cookieOpen = document.cookie
    .split("; ")
    .find((row) => row.startsWith(`${SIDEBAR_COOKIE_NAME}=`))
    ?.split("=")[1]

  if (cookieOpen != null) {
    return cookieOpen === "true"
  }

  return inMemorySidebarOpen ?? defaultOpen
}
...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() =>
    readSidebarOpenPreference(defaultOpen)
  )
  ...
  React.useEffect(() => {
    ...
    const restorePreference = window.setTimeout(() => {
      _setOpen(readSidebarOpenPreference(defaultOpen))
    }, 0)
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      try {
        ...
        // The cookie above is enough to preserve the UI preference.
      }
    },
    [setOpenProp, open]
  )
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[color,background-color,width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:gap-0 group-data-[collapsible=icon]:p-2! group-data-[collapsible=icon]:[&_svg]:size-4 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-[active=true]:bg-primary data-[active=true]:font-medium data-[active=true]:text-primary-foreground data-[active=true]:hover:bg-primary data-[active=true]:hover:text-primary-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
