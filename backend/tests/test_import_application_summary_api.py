import tempfile
import unittest
from pathlib import Path
from unittest.mock import patch

from fastapi import HTTPException

from backend.app.import_persistence import ImportPersistenceRepository
from backend.app.main import (
    app,
    apply_master_data_import,
    get_import_application_summary,
)
from backend.app.master_data_persistence import MasterDataPersistenceRepository
from backend.app.models import ImportBatchCreateRequest, ImportBatchRowResultInput


class ImportApplicationSummaryApiTest(unittest.TestCase):
    def test_import_application_summary_route_is_registered(self) -> None:
        routes = {(route.path, ",".join(sorted(route.methods))) for route in app.routes}

        self.assertIn(
            ("/api/v1/import-batches/{batch_id}/application-summary", "GET"),
            routes,
        )

    def test_import_application_summary_returns_not_applied_before_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            database_url = f"sqlite+pysqlite:///{Path(tmp_dir) / 'summary.db'}"
            import_repository = ImportPersistenceRepository(database_url)
            import_repository.init_schema()
            master_data_repository = MasterDataPersistenceRepository(database_url)
            master_data_repository.init_schema()
            _create_master_data_batch(import_repository, "BATCH-SUMMARY-001")

            with (
                patch(
                    "backend.app.main.get_import_persistence_repository",
                    return_value=import_repository,
                ),
                patch(
                    "backend.app.main.MasterDataPersistenceRepository",
                    return_value=master_data_repository,
                ),
            ):
                response = get_import_application_summary("BATCH-SUMMARY-001")

        self.assertEqual(response.batch_id, "BATCH-SUMMARY-001")
        self.assertEqual(response.file_type, "master_data")
        self.assertEqual(response.application_status, "not_applied")
        self.assertEqual(response.application_target, "master_data_snapshot")
        self.assertIsNone(response.import_version_id)
        self.assertEqual(response.applied_record_count, 0)

    def test_import_application_summary_returns_applied_after_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            database_url = f"sqlite+pysqlite:///{Path(tmp_dir) / 'summary.db'}"
            import_repository = ImportPersistenceRepository(database_url)
            import_repository.init_schema()
            master_data_repository = MasterDataPersistenceRepository(database_url)
            master_data_repository.init_schema()
            _create_master_data_batch(import_repository, "BATCH-SUMMARY-002")

            with (
                patch(
                    "backend.app.main.get_import_persistence_repository",
                    return_value=import_repository,
                ),
                patch(
                    "backend.app.main.MasterDataPersistenceRepository",
                    return_value=master_data_repository,
                ),
            ):
                apply_master_data_import("BATCH-SUMMARY-002")
                response = get_import_application_summary("BATCH-SUMMARY-002")

        self.assertEqual(response.application_status, "applied")
        self.assertEqual(response.application_target, "master_data_snapshot")
        self.assertEqual(response.applied_record_count, 6)

    def test_import_application_summary_returns_404_for_missing_batch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            database_url = f"sqlite+pysqlite:///{Path(tmp_dir) / 'summary.db'}"
            import_repository = ImportPersistenceRepository(database_url)
            import_repository.init_schema()

            with patch(
                "backend.app.main.get_import_persistence_repository",
                return_value=import_repository,
            ):
                with self.assertRaises(HTTPException) as raised:
                    get_import_application_summary("missing")

        self.assertEqual(raised.exception.status_code, 404)
        self.assertEqual(raised.exception.detail["error"]["code"], "IMPORT_BATCH_NOT_FOUND")


def _create_master_data_batch(
    repository: ImportPersistenceRepository,
    batch_id: str,
) -> None:
    def row(row_number: int, standard_fields: dict[str, str]) -> ImportBatchRowResultInput:
        return ImportBatchRowResultInput(
            row_number=row_number,
            row_status="success",
            source_key=(
                standard_fields.get("reference_id")
                or standard_fields.get("employee_id")
                or standard_fields.get("binding_id")
            ),
            raw_data={"standard_fields": standard_fields},
        )

    repository.create_import_batch(
        ImportBatchCreateRequest(
            batch_id=batch_id,
            file_name="master_data.csv",
            file_type="master_data",
            uploaded_by="数据管理员",
            business_date_from="2026-05-01",
            business_date_to="2026-12-31",
            rows=[
                row(1, _reference("supplier", "SUP-A", "供应商 A")),
                row(2, _reference("workplace", "SH-01", "上海职场")),
                row(3, _reference("project", "BOSCH-CS", "博西客服")),
                row(4, _reference("skill", "L1-CN", "中文一线")),
                row(
                    5,
                    {
                        "record_type": "employee",
                        "employee_id": "A-1001",
                        "employee_name": "张三",
                        "status": "active",
                        "effective_from": "2026-05-01",
                        "effective_to": "2026-12-31",
                    },
                ),
                row(
                    6,
                    {
                        "record_type": "binding",
                        "binding_id": "BIND-A-1001",
                        "employee_id": "A-1001",
                        "supplier_id": "SUP-A",
                        "workplace_id": "SH-01",
                        "project_id": "BOSCH-CS",
                        "skill_id": "L1-CN",
                        "effective_from": "2026-05-11",
                        "effective_to": "2026-06-30",
                    },
                ),
            ],
        )
    )


def _reference(record_type: str, reference_id: str, reference_name: str) -> dict[str, str]:
    return {
        "record_type": record_type,
        "reference_id": reference_id,
        "reference_name": reference_name,
        "status": "active",
        "effective_from": "2026-05-01",
        "effective_to": "2026-12-31",
    }


if __name__ == "__main__":
    unittest.main()
