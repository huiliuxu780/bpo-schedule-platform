...
class ReviewClosureServiceTest(unittest.TestCase):
    ...
    def test_write_review_closure_closes_existing_open_case(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            database_url = f"sqlite+pysqlite:///{Path(directory) / 'review-close.db'}"
            source_ids = _seed_review_sources(database_url)
            repository = ReviewPersistenceRepository(database_url)
            ...
            repository.add_evidence(
                ReviewEvidenceInput(
                    evidence_id="EVD-WRITE-EXISTING",
                    case_id="CASE-WRITE-EXISTING",
                    evidence_type="note",
                    evidence_uri="local://review/CASE-WRITE-EXISTING/note",
                    submitted_by="supervisor-01",
                    note="已有证据",
                )
            )
            repository.add_conclusion(
                ReviewConclusionInput(
                    conclusion_id="CON-WRITE-EXISTING",
                    case_id="CASE-WRITE-EXISTING",
                    conclusion_type="confirmed_gap",
                    risk_level="high",
        ...
        self.assertEqual(detail.case.case_id, "CASE-WRITE-EXISTING")
        self.assertEqual(len(detail.evidence), 1)
        self.assertEqual(len(detail.conclusions), 1)
        self.assertIsNotNone(detail.closure)
        self.assertEqual(detail.closure.closure_id, "CLO-WRITE-EXISTING")
