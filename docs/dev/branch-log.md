...
- branch_name: `codex/im234-import-center-medium-gate-split`
- base_main_commit: `stacked on codex/im233-review-case-medium-gate-split`
- stacked_on: `codex/im233-review-case-medium-gate-split`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: PM-confirmed continuation after IM233. Split import-center review-case preview/gap, version workbench, and batch apply action gates into medium-grained sub-gates, and keep the old files as thin entrypoints.
- allowed_files_check: the three old import-center test entry files, six new split test files, `scripts/check.sh`, current Harness files, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected test gate split, check.sh integration, and traceability updates only; no changes to `app/**`, `components/**`, `hooks/**`, `lib/**`, backend, package/lockfile, dependencies, navigation, UI, database, permissions, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor files.
- check_result: focused import-center medium gate split runs passed with 22/22 entry-plus-child tests and 11/11 child-only tests. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, import-center sub-gates, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
- base_main_commit: `stacked on codex/im227-import-center-batch-apply-split`
- stacked_on: `codex/im227-import-center-batch-apply-split`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: PM-confirmed continuation after IM227. Split the already-green master-data agent model gate into smaller list, detail, import, and action gates, and keep the old file as a thin entrypoint.
- allowed_files_check: `scripts/tests/master-data-maintenance-agent-model.test.mjs`, new master-data agent split tests, `scripts/check.sh`, current Harness files, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected test gate split, check.sh integration, and traceability updates only; no changes to `app/**`, `components/**`, `hooks/**`, `lib/**`, backend, package/lockfile, dependencies, navigation, UI, database, permissions, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor files.
- check_result: focused master-data agent split run passed with 16/16 tests including the thin entrypoint and four new files. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, split master-data agent gates, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM198 Simple Table Schedule Interval Slice

- branch_name: `codex/im198-simple-table-schedule-interval`
- base_main_commit: `pending stacked continuation from codex/im197-simple-table-first-slice`
- stacked_on: `codex/im197-simple-table-first-slice`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product Design preservation pass for Task 3. Migrate only `components/schedule-plan-interval-table.tsx` to the existing `SimpleTable` after IM197 validated the pattern.
- allowed_files_check: `components/schedule-plan-interval-table.tsx`, `scripts/tests/simple-table-structure.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected schedule interval table migration, focused structure-test extension, and traceability updates only; no page, route, business copy, data model, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/simple-table-structure.test.mjs` first failed because `components/schedule-plan-interval-table.tsx` still owned `useReactTable`, `flexRender`, and the shadcn Table render loop. After implementation, the focused structure test passed with 4 tests. `npm run typecheck`, `npm run lint`, `bash scripts/check-state.sh --strict`, and `git diff --check` passed. Browser smoke on `/schedule-plans/plan-20260511-suzhou-bosch-v1` confirmed the interval table headers, 8 rows, and no false empty state. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM199 Simple Table Risk Shift Slice

- branch_name: `codex/im199-simple-table-risk-shift`
- base_main_commit: `pending stacked continuation from codex/im198-simple-table-schedule-interval`
- stacked_on: `codex/im198-simple-table-schedule-interval`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product Design preservation pass for Task 3. Migrate only `components/schedule-risk-shift-table.tsx` to the existing `SimpleTable` after IM197 and IM198 validated the pattern.
- allowed_files_check: `components/schedule-risk-shift-table.tsx`, `scripts/tests/simple-table-structure.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected schedule risk shift table migration, focused structure-test extension, and traceability updates only; no page, route, business copy, data model, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/simple-table-structure.test.mjs` first failed because `components/schedule-risk-shift-table.tsx` still owned `useReactTable`, `flexRender`, and the shadcn Table render loop. After implementation, the focused structure test passed with 5 tests. `npm run typecheck`, `npm run lint`, `bash scripts/check-state.sh --strict`, shadcn grep checks, and `git diff --check` passed. Browser smoke on `/schedule-risks/risk-plan-20260511-suzhou-bosch-v1-10%3A00` confirmed the shift table headers, 1 row, and no false empty state. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM200 Simple Table Risk Unavailability Slice

- branch_name: `codex/im200-simple-table-risk-unavailability`
- base_main_commit: `pending stacked continuation from codex/im199-simple-table-risk-shift`
- stacked_on: `codex/im199-simple-table-risk-shift`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product Design preservation pass for Task 3. Migrate only `components/schedule-risk-unavailability-table.tsx` to the existing `SimpleTable` after IM197 through IM199 validated the pattern.
- allowed_files_check: `components/schedule-risk-unavailability-table.tsx`, `scripts/tests/simple-table-structure.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected schedule risk unavailability table migration, focused structure-test extension, and traceability updates only; no page, route, business copy, data model, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/simple-table-structure.test.mjs` first failed because `components/schedule-risk-unavailability-table.tsx` still owned `useReactTable`, `flexRender`, and the shadcn Table render loop. After implementation, the focused structure test passed with 6 tests. `npm run typecheck`, `npm run lint`, `bash scripts/check-state.sh --strict`, shadcn grep checks, and `git diff --check` passed. Browser smoke on `/schedule-risks/risk-plan-20260511-suzhou-bosch-v1-10%3A00` confirmed the unavailability table headers, 1 row, and no false empty state. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM201 Simple Table Unavailability Impact Shift Slice

- branch_name: `codex/im201-simple-table-unavailability-impact-shift`
- base_main_commit: `pending stacked continuation from codex/im200-simple-table-risk-unavailability`
- stacked_on: `codex/im200-simple-table-risk-unavailability`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product Design preservation pass for Task 3. Migrate only `components/unavailability-impact-shift-table.tsx` to the existing `SimpleTable` after IM197 through IM200 validated the pattern.
- allowed_files_check: `components/unavailability-impact-shift-table.tsx`, `scripts/tests/simple-table-structure.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected unavailability impact shift table migration, focused structure-test extension, and traceability updates only; no page, route, business copy, data model, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/simple-table-structure.test.mjs` first failed because `components/unavailability-impact-shift-table.tsx` still owned `useReactTable`, `flexRender`, and the shadcn Table render loop. After implementation, the focused structure test passed with 7 tests. `npm run typecheck`, `npm run lint`, `bash scripts/check-state.sh --strict`, shadcn grep checks, and `git diff --check` passed. Browser smoke on `/unavailability/unavail-20260511-001` confirmed the impact shift table headers, 2 rows, plan links, and no false empty state. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM202 Simple Table Unavailability Impact Risk Slice

- branch_name: `codex/im202-simple-table-unavailability-impact-risk`
- base_main_commit: `pending stacked continuation from codex/im201-simple-table-unavailability-impact-shift`
- stacked_on: `codex/im201-simple-table-unavailability-impact-shift`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product Design preservation pass for Task 3. Migrate only `components/unavailability-impact-risk-table.tsx` to the existing `SimpleTable` after IM197 through IM201 validated the pattern.
- allowed_files_check: `components/unavailability-impact-risk-table.tsx`, `scripts/tests/simple-table-structure.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected unavailability impact risk table migration, focused structure-test extension, and traceability updates only; no page, route, business copy, data model, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/simple-table-structure.test.mjs` first failed because `components/unavailability-impact-risk-table.tsx` still owned `useReactTable`, `flexRender`, and the shadcn Table render loop. After implementation, the focused structure test passed with 8 tests. `npm run typecheck`, `npm run lint`, `bash scripts/check-state.sh --strict`, shadcn grep checks, and `git diff --check` passed. Browser smoke on `/unavailability/unavail-20260511-001` confirmed the impact risk table headers, 1 row, detail link, and no false empty state. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
- remote_status: `pushed to origin/codex/im203-simple-table-shift-details after PM continuation; remote head 924a546f82b653aeb9921b0d0168c060dd26436e.`
...
### IM204 MainTableShell Boundary Spec
...
- remote_status: `pushed to origin/codex/im207-main-table-shell-unavailability after PM continuation; remote head fe82af0a0df2587af330d9262721ef690e0dc8b4 includes IM207 plus local-artifact ignore cleanup.`
...
### IM208 MainTableShell Closeout

- branch_name: `codex/im208-main-table-shell-closeout`
- base_main_commit: `66193b1d694dabcd7621935262fbcddc7a0d2fa7`
- stacked_on: `codex/im207-main-table-shell-unavailability`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Documentation and Harness closeout for the SimpleTable/MainTableShell table-abstraction chain. Confirm that `components/data-table.tsx` stays deferred until dashboard anomaly product ownership, route responsibility, and workflow value are clear.
- allowed_files_check: current queue and active tasks, current project context, raw requirements, user stories, backlog, task log, audit report, project state, branch log, and `docs/design/main-table-shell-boundary-spec.md` only.
- scope_diff_check: expected documentation/state closeout only; no `app/**`, `components/**`, `hooks/**`, `lib/**`, backend, database, dependency, package/lockfile, route, data, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes.
- check_result: `bash scripts/check-state.sh --strict` passed with current context line budget 160/160 and empty current queue/active tasks. `git diff --check` passed. Full `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
...
### IM209 Dashboard Anomaly Ownership Audit

- branch_name: `codex/im209-dashboard-anomaly-ownership-audit`
- base_main_commit: `66193b1d694dabcd7621935262fbcddc7a0d2fa7`
- stacked_on: `codex/im208-main-table-shell-closeout`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product/design audit of `/dashboard` anomaly table ownership. Confirm whether `components/data-table.tsx` should stay an overview widget, become an exception triage entry, or be replaced by downstream summary links.
- allowed_files_check: current queue and active tasks, current project context, raw requirements, user stories, backlog, task log, audit report, project state, branch log, and `docs/design/dashboard-anomaly-table-ownership-audit.md` only.
- scope_diff_check: expected documentation/state audit only; no `app/**`, `components/**`, `hooks/**`, `lib/**`, backend, database, dependency, package/lockfile, route, data, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes.
- check_result: `bash scripts/check-state.sh --strict` passed with current context line budget 160/160 and empty current queue/active tasks. `git diff --check` passed. Full `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: pending
- integration_status: `not_started`
- integration_method: `N/A`
...
### IM210 Dashboard Downstream Entry Spec

- branch_name: `codex/im210-dashboard-downstream-entry-spec`
- base_main_commit: `66193b1d694dabcd7621935262fbcddc7a0d2fa7`
- stacked_on: `codex/im209-dashboard-anomaly-ownership-audit`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product/design specification for future dashboard anomaly downstream entries. Define dashboard rows as summary-to-workspace links only, using existing comparison-run, review-case, import-batch, actual-log production, and schedule production detail workspaces.
- allowed_files_check: current queue and active tasks, current project context, raw requirements, user stories, backlog, task log, audit report, project state, branch log, and `docs/design/dashboard-downstream-entry-spec.md` only.
- scope_diff_check: expected documentation/state specification only; no `app/**`, `components/**`, `hooks/**`, `lib/**`, backend, database, dependency, package/lockfile, route, query-contract, data, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes.
- check_result: `git diff --check` passed after adding the new design spec to the diff. `bash scripts/check-state.sh --strict` passed twice: once with IM210 in current execution state and once after current queue/active tasks returned to empty. Full `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: pending
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM213 Project Understanding Requirements Calibration

- branch_name: `codex/im213-requirements-baseline-calibration`
- base_main_commit: `66193b1d694dabcd7621935262fbcddc7a0d2fa7`
- stacked_on: `codex/im212-dashboard-chain-closeout`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Product/design calibration for the 2026-06-14 external project understanding and requirements document. Keep it as a product baseline, mark stale status claims, and prevent old P0/P1 lists from becoming automatic backlog.
- allowed_files_check: current queue and active tasks, current project context, raw requirements, user stories, backlog, task log, audit report, project state, branch log, and `docs/design/project-understanding-requirements-calibration.md` only.
- scope_diff_check: expected documentation/state calibration only; no `app/**`, `components/**`, `hooks/**`, `lib/**`, backend, database, dependency, package/lockfile, route, query-contract, data, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes.
- check_result: `git diff --check` passed after adding the new calibration document to the diff. `bash scripts/check-state.sh --strict` passed twice: once with IM213 in current execution state and once after current queue/active tasks returned to empty. Full `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: pending
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
- remote_status: `not_pushed; pending PM push decision after local closeout commit.`
- scope: QA acceptance smoke for current review-case workspace using only the already-running `127.0.0.1:3000` environment, then PM-approved no-new-env model/contract-only closeout.
...
- check_result: Live smoke evidence: 3000 review-case list, processing-stage filters, `CASE-QUERY-001` detail, `?evidence=failed`, and `?closure=success` URLs returned 200, but pages rendered `fetch failed` because direct 8000 review-case API calls could not connect. PM then continued under the no-new-env constraint, so IM215 was closed as model/contract-only QA record rather than live seeded acceptance. Focused helper tests passed: `node --test scripts/tests/frontend-api-utilities.test.mjs` and `node --test scripts/tests/shared-empty-state.test.mjs`. Direct `import-center-model.test.mjs` is not pass evidence because it is currently blocked at TS/ESM import resolution; broad `product-structure.test.mjs` is not pass evidence because it has unrelated master-data assertion failures. Final `git diff --check`, `bash scripts/check-state.sh --strict`, and full `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed after closeout docs, including strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests.
- local_commit_sha: pending
...
### IM221 Import Center Batch Template Test Split

- branch_name: `codex/im221-import-center-batch-template-test-split`
- base_main_commit: `56835b195f2ae67ec2567f4bdf4915b282901a72`
- stacked_on: `codex/im220-import-center-test-gate-final-split`
- remote_status: `not_pushed; pending PM push decision after local commit.`
- scope: Continue import-center test gate splitting by decomposing the green batch/template model assertions into narrower executable files.
- allowed_files_check: `scripts/tests/import-center-batch-template-model.test.mjs`, `scripts/tests/import-center-batch-apply-model.test.mjs`, `scripts/tests/import-center-template-model.test.mjs`, `scripts/tests/import-center-upload-model.test.mjs`, `scripts/tests/import-center-batch-detail-model.test.mjs`, `scripts/check.sh`, current project context, raw requirements, user stories, backlog, task log, audit report, project state, and branch log only.
- scope_diff_check: expected test-file split, old batch/template test deletion, check.sh wiring, and Harness traceability only; no `app/**`, `components/**` implementation changes, `hooks/**`, `lib/**`, backend, database, dependency, package/lockfile, route, query-contract, data, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes.
- check_result: Focused split batch/template tests passed with 27/27 tests. `product-structure.test.mjs` original baseline was checked and currently passes only 21/35 tests, so it was not connected to the default gate. `npm run lint`, `bash scripts/check-state.sh --strict`, `git diff --check`, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed.
- local_commit_sha: pending
- integration_status: `done`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
- base_main_commit: `ee11b66`
- stacked_on: `codex/im188-forecast-version-entry-semantics`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: PM-confirmed continuation after IM188. Clarify schedule-version entry semantics on the existing personnel schedule version list/detail route without adding routes or write capabilities.
- allowed_files_check: `app/schedule-plans/production/[batchId]/page.tsx`, `components/personnel-schedule-production-model.ts`, `components/personnel-schedule-production-workbench.tsx`, personnel-schedule focused tests, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected schedule-version copy/model semantics, focused regression test, and traceability updates only; no new route, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/personnel-schedule-version-entry-semantics.test.mjs` first failed because list action and detail return/copy still used old semantics; after implementation, the new structure test passed. Existing `node --test scripts/tests/personnel-schedule-production-model.test.mjs` passed, `npm run typecheck` passed, `npm run lint` passed, and HTTP smoke on `/schedule-plans/production` plus `/schedule-plans/production/BATCH-SCH-001` returned 200 with new version semantics. `bash scripts/check-state.sh --strict` and `git diff --check` passed, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
- base_main_commit: `3d8fa69`
- stacked_on: `codex/im175-action-runtime-guards`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Recovery-plan Stage 2 first implementation slice. Add `app/error.tsx` global error boundary with shadcn Alert/Button recovery controls and a structural regression test.
- allowed_files_check: `app/error.tsx`, `scripts/tests/global-error-boundary.test.mjs`, current project context, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected global error boundary, structural test, and traceability updates only; no route-group migration, route loading files, business page changes, navigation changes, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes.
- check_result: TDD RED `node --test scripts/tests/global-error-boundary.test.mjs` first failed because `app/error.tsx` was missing. After implementation, the same structure test passed, `npm run typecheck` passed, `npm run lint` passed, `bash scripts/check-state.sh --strict` passed, `git diff --check` passed, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM178 Import Center Model First Split

- branch_name: `codex/im178-import-center-model-first-split`
- base_main_commit: `faebb34`
- stacked_on: `codex/im177-core-route-loading-states`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Recovery-plan Stage 3 first implementation slice. Mechanically split import-center model types, formatters, and navigation helpers into dedicated component model files while keeping `components/import-center-model.ts` as the legacy compatibility entrypoint.
- allowed_files_check: `components/import-center-model.ts`, `components/import-center-types.ts`, `components/import-center-formatters.ts`, `components/import-center-navigation.ts`, `scripts/tests/import-center-model-first-split.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected model split files, compatibility re-exports, structural test, and traceability updates only; no visible UI, navigation, API URL semantic, response-shape, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/import-center-model-first-split.test.mjs` first failed because `components/import-center-types.ts` was missing. After implementation, the structure test passed, `npm run typecheck` passed, and `npm run lint` passed. `bash scripts/check-state.sh --strict` passed, `git diff --check` passed, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK. Prettier formatting command failed because the repo `.prettierrc` references missing `prettier-plugin-tailwindcss`; no package changes were made in this slice.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM180 Master Data Workbench Split

- branch_name: `codex/im180-master-data-workbench-split`
- base_main_commit: `6fb0979`
- stacked_on: `codex/im179-import-center-summary-split`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Recovery-plan Stage 3 third implementation slice. Mechanically split the master-data maintenance workbench into responsibility files and keep `components/master-data-maintenance-workbench.tsx` as the legacy compatibility entrypoint.
- allowed_files_check: `components/master-data-maintenance-workbench.tsx`, six new master-data maintenance responsibility files, `scripts/tests/master-data-workbench-split.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected responsibility UI split, compatibility re-exports, structural test, and traceability updates only; no visible UI, route, interaction, business semantic, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/master-data-workbench-split.test.mjs` first failed because the responsibility files were missing. After implementation, the structure test passed, `npm run typecheck` passed, and `npm run lint` passed. `bash scripts/check-state.sh --strict` passed, `git diff --check` passed, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM181 Master Data Model Split

- branch_name: `codex/im181-master-data-model-split`
- base_main_commit: `731bf88`
- stacked_on: `codex/im180-master-data-workbench-split`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Recovery-plan Stage 3 fourth implementation slice. Mechanically split the master-data maintenance model into responsibility files and keep `components/master-data-maintenance-model.ts` as the legacy compatibility entrypoint.
- allowed_files_check: `components/master-data-maintenance-model.ts`, eight new master-data maintenance model responsibility files, `scripts/tests/master-data-model-split.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected responsibility model split, compatibility re-exports, structural test, and traceability updates only; no visible UI, route, interaction, business semantic, API URL, response-shape, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/master-data-model-split.test.mjs` first failed because the responsibility files were missing. After implementation, the structure test passed, `npm run typecheck` passed, and `npm run lint` passed. `bash scripts/check-state.sh --strict` passed, `git diff --check` passed, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM184 Master Data Form Feedback Consistency

- branch_name: `codex/im184-form-feedback-consistency`
- base_main_commit: `66193b1d694dabcd7621935262fbcddc7a0d2fa7`
- stacked_on: `codex/im183-shared-empty-state`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Recovery-plan Stage 4 third implementation slice. Centralize master-data maintenance form required markers and submit pending feedback without adding business capability.
- allowed_files_check: `components/maintenance-submit-button.tsx`, `components/master-data-maintenance-fields.tsx`, `components/master-data-maintenance-forms.tsx`, `scripts/tests/master-data-form-feedback.test.mjs`, current Harness files, registry trace index, raw requirements, user stories, task log, audit report, project state, backlog, and this branch-log entry only.
- scope_diff_check: expected shared submit component, required-label marker, submit-button replacement, structure test, and traceability updates only; no new business field, button, route, navigation, backend, database, dependency, package/lockfile, permission, approval, export, batch-operation, automatic-scheduling, formula, settlement, supplier-contract, minimum-staffing, or charge-factor changes. `.local/`, `.qoder/`, and the pre-existing unrelated `docs/design/shadcn-dashboard-01-visual-alignment-report.md` deletion must not be staged.
- check_result: TDD RED `node --test scripts/tests/master-data-form-feedback.test.mjs` first failed because the shared submit component and required marker did not exist and forms still used raw submit Buttons. After implementation, the structure test passed, `npm run typecheck` passed, `npm run lint` passed, `bash scripts/check-state.sh --strict` passed, browser smoke on `/master-data/agents/new` confirmed shared submit, required markers, and no console errors, `git diff --check` passed, and final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with strict state, shadcn gate, lint, typecheck, Next build, and backend 215 tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM253 Schedule Plan Readiness

- branch_name: `codex/im253-schedule-plan-readiness`
- base_main_commit: `stacked on pushed codex/im252-operational-runtime-acceptance at 83eb1e2`
- stacked_on: `codex/im252-operational-runtime-acceptance`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Medium frontend-scaffold block for schedule-plan list/detail readiness. Add operator-facing data-source messaging, stable empty-state distinction, detail data-source handling, and read-only downstream entries for related risks and unavailability records.
- qoder_mode: `true; Qoder implemented Packet A/B under bounded file scopes. Codex reviewed diffs, fixed detail 404/fallback semantics, owns Harness updates, final verification, local commit, and push decision.`
- allowed_files_check: schedule plan list/detail frontend files, schedule plan table, schedule plan client/model file, focused schedule-plan readiness test, current Harness context, audit report, task log, and branch log only.
- scope_diff_check: expected schedule-plan readiness UI/model/test and traceability updates only; no backend API, dependency, package/lockfile, schema, migration, auth, permission, approval, export, batch-operation, automatic scheduling, production formula, settlement, charge-factor, or external integration changes.
- check_result: Qoder Packet A/B diffs were reviewed by Codex. Codex corrected detail result semantics so API 404 does not fall back to local seed data. Focused `node --test scripts/tests/schedule-plan-readiness-model.test.mjs` passed with 26 tests. Focused `node --test scripts/tests/*schedule-plan*.test.mjs` passed with 41 tests. `npm run lint`, `npm run typecheck`, `bash scripts/check-state.sh --strict`, and `git diff --check` passed. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with 621 Node tests (620 pass, 1 skipped), shadcn check, lint, typecheck, Next build, and 241 backend unittest tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM255 Schedule Plan Fulfillment Preview

- branch_name: `codex/im255-schedule-plan-fulfillment-preview`
- base_main_commit: `stacked on pushed codex/im254-dashboard-visual-polish at 045d2b0`
- stacked_on: `codex/im254-dashboard-visual-polish`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Medium frontend-scaffold block for schedule-plan detail fulfillment preview. Show bounded related risk and overlapping unavailability previews using existing local data only.
- qoder_mode: `true; Qoder implemented the medium packet under bounded file scopes. Codex reviewed diffs, fixed encoded route links and preview copy completeness, owns Harness updates, final verification, local commit, and push decision.`
- allowed_files_check: schedule plan detail page, schedule plan model/client file, focused schedule-plan fulfillment preview test, current Harness context, audit report, task log, and branch log only.
- scope_diff_check: expected schedule-plan fulfillment preview model/UI/test and traceability updates only; no backend API, dependency, package/lockfile, schema, migration, auth, permission, approval, export, batch-operation, automatic scheduling, production formula, settlement, charge-factor, or external integration changes.
- check_result: Qoder diff was reviewed by Codex. Codex corrected route encoding for preview links and ensured risk recommendation plus unavailability note are visible in the preview rows. Focused `node --test scripts/tests/schedule-plan-fulfillment-preview-model.test.mjs` passed with 16 tests. Focused `node --test scripts/tests/*schedule*fulfillment*.test.mjs scripts/tests/*schedule-plan*.test.mjs` passed with 76 tests. `npm run typecheck`, `bash scripts/check-state.sh --strict`, and `git diff --check` passed. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with 664 Node tests (663 pass, 1 skipped), shadcn check, lint, typecheck, Next build, and 241 backend unittest tests OK.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM259 Operational Source Consistency

- branch_name: `codex/im259-operational-source-consistency`
- base_main_commit: `stacked on pushed codex/im258-operational-runtime-acceptance at 263589e`
- stacked_on: `codex/im258-operational-runtime-acceptance`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Medium frontend-scaffold block for operational data-source consistency. Add result-style source messaging to schedule risk detail, unavailability list/detail, and shift details.
- qoder_mode: `true; Qoder implemented the medium packet and obeyed no-commit/no-push instructions. Codex reviewed the staged diff, fixed source-aware empty-state gaps, owns Harness updates, final verification, local commit, and push decision.`
- allowed_files_check: four operational pages, schedule plan/unavailability client files, small table empty-message props, focused IM259 test, current Harness context, audit report, task log, and branch log only.
- scope_diff_check: expected source messaging model/UI/test and traceability updates only; no backend API, dependency, package/lockfile, schema, migration, auth, permission, approval, export, batch-operation, automatic scheduling, production formula, settlement, charge-factor, or external integration changes.
- check_result: Focused `node --test scripts/tests/im259-operational-source-consistency.test.mjs` passed with 17 tests. Focused related `node --test scripts/tests/im259-operational-source-consistency.test.mjs scripts/tests/*schedule*risk*.test.mjs scripts/tests/*unavailability*.test.mjs scripts/tests/dashboard-risk-unavailability-model.test.mjs` passed with 35 tests. `npm run typecheck` and `git diff --check` passed. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with 697 Node tests (696 pass, 1 skipped), shadcn check, lint, typecheck, Next build, 241 backend unittest tests OK, and project Harness check passed.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM262 Schedule Plan Draft Workflow Closeout

- branch_name: `codex/im262-schedule-plan-draft-workflow-closeout`
- base_main_commit: `stacked on pushed codex/im261-operational-browser-acceptance at 7ff1b96`
- stacked_on: `codex/im261-operational-browser-acceptance`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Medium frontend-scaffold block for schedule-plan draft workflow UI closeout. Extract shared create/edit draft form, add save-before-review summary, preserve feedback and non-draft blockers, and update acceptance record.
- qoder_mode: `true; Qoder implemented the medium packet and obeyed no-commit/no-push instructions. Codex reviewed the diff, corrected summary gap semantics and acceptance wording, owns Harness updates, final verification, local commit, and push decision.`
- allowed_files_check: schedule-plan new/edit pages, shared draft form and summary components, focused draft workflow tests, IM260 focused test update, acceptance report, current Harness context, audit report, task log, and branch log only.
- scope_diff_check: expected schedule-plan draft form/summary UI, focused tests, acceptance documentation, and traceability updates only; no backend API, dependency, package/lockfile, schema, migration, auth, permission, approval, export, batch-operation, automatic scheduling, production formula, settlement, charge-factor, or external integration changes.
- check_result: Focused `node --test scripts/tests/schedule-plan-draft-workflow-closeout-model.test.mjs scripts/tests/schedule-plan-draft-hardening-model.test.mjs` passed with 43 tests. Focused `node --test scripts/tests/*schedule-plan*.test.mjs` passed with 100 tests. `npm run lint`, `npm run typecheck`, `node scripts/check-shadcn-ui.mjs`, `git diff --check`, and `bash scripts/check-state.sh --strict` passed. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with 740 Node tests (739 pass, 1 skipped), shadcn check, lint, typecheck, Next build, 241 backend unittest tests OK, and project Harness check passed.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM263 Dashboard Operational Filter Console

- branch_name: `codex/im263-dashboard-operational-filter-console`
- base_main_commit: `stacked on pushed codex/im262-schedule-plan-draft-workflow-closeout at 9f058bb`
- stacked_on: `codex/im262-schedule-plan-draft-workflow-closeout`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Medium frontend-scaffold block for the dashboard operational filter console. Replace decorative dashboard filter buttons with URL-driven project/site/plan status filtering that feeds the shared operational view model, while keeping table-level filters inside the anomaly table.
- qoder_mode: `true; Qoder implemented the medium packet and obeyed no-commit/no-push instructions. Codex reviewed the diff, corrected the over-scoped header filter design, removed keyword/risk-level/issue-status from the dashboard header, tightened plan-status data scoping, and owns Harness updates, final verification, local commit, and push decision.`
- allowed_files_check: dashboard page, dashboard model, global filter bar, trend chart boundary copy, focused dashboard tests, current Harness context, audit report, task log, and branch log only.
- scope_diff_check: expected dashboard filter UI/model/test and traceability updates only; no backend API, dependency, package/lockfile, schema, migration, auth, permission, approval, export, batch-operation, automatic scheduling, production formula, settlement, charge-factor, or external integration changes.
- check_result: Focused `node --test scripts/tests/dashboard-operational-filter-console.test.mjs` passed with 11 tests. Focused `node --test scripts/tests/*dashboard*.test.mjs` passed with 176 tests. `npm run lint`, `npm run typecheck`, `node scripts/check-shadcn-ui.mjs`, `bash scripts/check-state.sh --strict`, and `git diff --check` passed before final Harness verification. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with 751 Node tests (750 pass, 1 skipped), shadcn check, lint, typecheck, Next build, 241 backend unittest tests OK, and project Harness check passed.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
...
### IM270 Dashboard 01 Restoration

- branch_name: `codex/im270-dashboard-01-restoration`
- base_main_commit: `stacked on local codex/im269-dashboard-card-visual-alignment at 6a5c9c1`
- stacked_on: `codex/im269-dashboard-card-visual-alignment`
- remote_status: `not_pushed; local branch until final check/commit and PM push confirmation.`
- scope: Medium frontend-scaffold restoration for `/dashboard` and global shell after PM rejected piecemeal shadcn styling. Restore the dashboard-01 structure across sidebar, KPI cards, full-width chart, and table workbench while preserving BPO business data.
- qoder_mode: `false; PM explicitly criticized the existing implementation and Codex performed the restoration directly.`
- allowed_files_check: dashboard page, app sidebar, dashboard chart/table/card components, shared card slot primitive, focused dashboard/global-shell tests, current Harness context, audit report, task log, and branch log only.
- scope_diff_check: expected dashboard-01 structural restoration only; no backend API, dependency, package/lockfile, schema, migration, auth, permission, approval, export, batch-operation, automatic scheduling, production formula, settlement, charge-factor, external integration, or new pseudo dashboard controls.
- check_result: Focused dashboard/global-shell run `node --test scripts/tests/*dashboard*.test.mjs scripts/tests/product-structure-global-shell.test.mjs` passed with 192 tests after removing stale heatmap-first-screen assertions. The initial full check exposed stale sidebar tests and a real sidebar scope issue: `导入中心` had been reintroduced as a product entry. Codex removed that sidebar entry, updated current-route navigation tests for `/shift-details` and `/unavailability`, and reran focused navigation checks successfully. Final `BPO_NODE22_BIN=/opt/homebrew/opt/node@22/bin bash scripts/check.sh` passed with 767 Node tests (766 pass, 1 skipped), shadcn check, lint, typecheck, Next build, 241 backend unittest tests OK, and project Harness check passed.
- visual_evidence: Browser screenshot capture was attempted but produced a black/unusable image; no screenshot evidence is claimed for this task.
- local_commit_sha: to be reported in Done Report after local commit creation
- integration_status: `not_started`
- integration_method: `N/A`
- integration_commit_sha: `N/A`
