...
### 2026-06-29 - IM270 经营总览 dashboard-01 结构还原

#### 审计计划

- 承接 PM 对“假改”、导航交互、重复图标和卡片结构的强反馈，把本轮限定为一个中等 dashboard-01 restoration，而不是继续做零散样式补丁。
- 以官方 shadcn dashboard-01 结构为基线：sidebar frame、metric cards、full-width chart、tabs table rhythm；保留 BPO 业务数据和真实可达路由。
- 不新增依赖、不切换 preset、不覆盖 package/lockfile、不新增后端/API/数据库/权限/审批/导出/批量/自动排班/生产公式/结算/收费因子。

#### 执行结果

- `components/app-sidebar.tsx` 重建为 dashboard-01 风格的 offcanvas sidebar：品牌区、快速新建、待处理风险、扁平主导航、运营数据、主数据和用户区。主数据条目改用 `Users`、`Network`、`MapPin`、`Handshake`、`Wrench` 等不同图标，删除重复 group icon 模式。
- `components/ui/card.tsx` 补齐 shadcn dashboard-01 需要的 `CardAction` slot，并让 `CardHeader` 支持 action 右侧布局。
- `components/section-cards.tsx` 改用 `CardAction`、`CardFooter`、dashboard-01 metric-card grid、`shadow-xs` 和底部 insight/note rhythm；保留整卡 drilldown，不添加伪按钮。
- `components/chart-area-interactive.tsx` 改为 shadcn `ChartContainer` + `ChartTooltip` 的全宽趋势图，删除伪时间切换控件，仅保留“示例数据”边界。
- `/dashboard` 首屏移除挤压主内容的 side heatmap，改为 KPI -> full-width chart -> table workbench；heatmap view-model 保留但不再渲染在首屏。
- `components/data-table.tsx` 保留真实的异常表筛选和列控制，但回到 dashboard-01 的 tabs/toolbar/table spacing。
- 更新 dashboard/global-shell 测试，保护 offcanvas sidebar、distinct master-data icons、CardAction/CardFooter、full-width chart、无首屏 side heatmap、无顶部 filter console、无实现口径 banner。
