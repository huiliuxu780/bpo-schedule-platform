...
After IM133, single-agent maintenance could update core employee fields, but employee multi-skill rows were still import/list only. `US754/IM134` is complete. The backend now supports a single employee skill-set `replace` maintenance call that validates source batch, employee, skills, and effective period through the existing master-data persistence rules. `/master-data/agents` now defaults to a customer-service personnel management list with search filters, a guarded bulk toolbar, and row-level edit/freeze/skill-maintenance entries; large always-visible submit tabs are not the default personnel experience.
