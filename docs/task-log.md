...
- story_ids:
  - `US606`
- action: 在履约日历实际异常面板展示 actual-log 切片和排班对比异常。
- status: `done`
- notes: `/person-timeline` 小组成员单日矩阵右侧异常面板现在展示“实际履约切片”，包含切片数量、质量问题、排班对比异常、影响分钟、来源记录、登录/状态/有效产能分钟和质量问题说明。浏览器 smoke 已确认选中 `A-1002::late_login` 时可见“实际履约切片”“排班对比异常”“来源记录”“登录时间晚于排班开始”。本次不新增独立状态页、不改后端、不新增依赖，不做数据库、真实外部系统、权限、审批、导出、批量或生产公式；current queue 已推进到 Q124。
...
- story_ids:
  - `US597`
- action: 完成 G001 需求预测与排班对齐。
- status: `done`
- notes: 导入预测现在可按业务日、职场、项目、0.5h 时段、技能组和等级与人员排班 0.5h 汇总对齐，输出缺口、超排、平衡和无匹配排班候选，并保留预测批次/版本、排班版本、排班来源批次、排班明细和员工追溯。没有新增数据库、ORM、migration、真实外部接口、依赖、权限、审批、导出、批量、文件存储、Excel xlsx 解析、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已从 US597/F411 切到 US598/Q121。
...
- story_ids:
  - `US596`
- action: 完成 G001 需求预测版本与变更追踪。
- status: `done`
- notes: 同一业务日、职场、项目、0.5h 时段、技能组和等级再次导入需求预测时，会生成本地版本变更记录，保留上一来源批次、上一版本、新来源批次、新版本、前后预测人数和变更类型；不会静默覆盖来源。没有新增数据库、ORM、migration、真实外部接口、依赖、权限、审批、导出、批量、文件存储、Excel xlsx 解析、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已从 US596/F410 切到 US597/F411。
...
- story_ids:
  - `US593`
- action: 完成 G001 人员级排班导入班次引用与版本。
- status: `done`
- notes: 人员级排班 CSV 导入现在会在 process-memory 中保留人员排班业务记录，记录来源批次、排班版本、班次引用状态、员工、业务日、职场、供应商、项目、技能、班次和开始结束时间；未知班次类型进入失败行，不进入业务记录。排班计划页展示人员排班导入、来源批次、排班版本和班次引用。没有新增数据库、ORM、migration、真实外部接口、依赖、权限、审批、导出、批量、文件存储、Excel xlsx 解析、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已移除 US593/F407，下一项为 US594/F408。
...
- story_ids:
  - `US591`
- action: 完成 G001 主数据维护状态有效期与引用校验。
- status: `done`
- notes: 主数据现在支持 process-memory 新增、修改、冻结、解冻、有效期和绑定关系引用校验；冻结、停用、过期、缺失或绑定不一致会返回数据质量式错误码，不进入正常引用。主数据页展示维护状态、新增或修改、冻结、解冻和引用校验入口。没有新增数据库、ORM、migration、真实外部接口、依赖、权限、审批、导出、批量、文件存储、Excel xlsx 解析、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已移除 US591/F406，下一项为 US592/Q119。
...
- story_ids:
  - `US581-US583`
- action: 完成数据质量交接风险关联导入批次影响。
- status: `done`
- notes: 数据质量总览页现在展示“交接风险关联导入批次影响”，把 owner 交接风险继续关联到本地导入批次和失败行，包含 owner、代表问题、关联批次、失败行、字段、影响对象、质量详情入口、批次入口和人员履约入口，并提供下一查看提示和暂缓能力；模型基于现有本地 owner 交接风险和 fallback 导入批次只读聚合。本次不新增真实上传、后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US578-US580`
- action: 完成数据质量分组步骤 owner 交接风险摘要。
- status: `done`
- notes: 数据质量总览页现在展示“分组步骤 owner 交接风险摘要”，把 owner 交接摘要转成主管可读的阻塞原因，包含 owner、代表问题、代表人员、关联分组、风险原因、质量详情入口和人员履约入口，并提供下一查看提示和暂缓能力；模型基于现有本地分组步骤 owner 交接摘要只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US572-US574`
- action: 完成数据质量分组步骤 owner 复核队列。
- status: `done`
- notes: 数据质量总览页现在展示“分组步骤 owner 复核队列”，把 owner/人员负载转成 rank、owner、代表问题、代表人员、关联分组、队列理由、质量详情入口和人员履约入口，并提供下一查看提示和暂缓能力；模型基于现有本地分组步骤 owner/人员负载只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US569-US571`
- action: 完成数据质量分组步骤 owner/人员负载摘要。
- status: `done`
- notes: 数据质量总览页现在展示“分组步骤 owner/人员负载”，把分组复核步骤按 owner 汇总为步骤数、影响人员、关联分组、代表问题、质量详情入口和人员履约入口，并提供下一查看提示和暂缓能力；模型基于现有本地分组步骤影响对象只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US566-US568`
- action: 完成数据质量分组步骤影响对象摘要。
- status: `done`
- notes: 数据质量总览页现在展示“分组步骤影响对象”，把分组复核步骤落到代表问题、影响人员、影响对象、质量详情入口和人员履约入口，并提供下一查看提示和暂缓能力；模型基于现有本地分组复核顺序和数据质量问题只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US563-US565`
- action: 完成数据质量分组复核顺序摘要。
- status: `done`
- notes: 数据质量总览页现在展示“质量分组复核顺序”，把影响履约异常的质量分组排成主管查看步骤，并展示首要分组、owner、风险、代表问题、影响异常、影响人员、阻断行、分组入口、下一查看提示和暂缓能力；模型基于现有本地分组异常影响覆盖只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US560-US562`
- action: 完成数据质量分组异常影响覆盖摘要。
- status: `done`
- notes: 数据质量总览页现在展示“质量分组异常影响覆盖”，从质量分组反查影响履约异常的数据质量问题，并展示影响分组、影响异常、影响人员、阻断行、首要分组、代表问题、模板、字段、影响对象、分组入口和暂缓能力；模型基于现有本地 fallback 质量分组和数据质量问题只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US557-US559`
- action: 完成数据质量复核建议质量分组摘要。
- status: `done`
- notes: 数据质量总览页现在展示“复核建议质量分组”，把下一轮复核建议的代表问题串到质量分组、风险、owner、模板、字段和分组入口，并提供下一查看提示和暂缓能力；模型基于现有本地下一轮复核建议和 fallback 质量分组只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US554-US556`
- action: 完成数据质量复核建议导入批次影响摘要。
- status: `done`
- notes: 数据质量总览页现在展示“复核建议导入批次影响”，把下一轮复核建议的代表问题串到导入批次、失败行、匹配字段和影响对象，并提供关联批次入口和暂缓能力；模型基于现有本地建议问题、数据质量问题和 fallback 导入批次影响只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US551-US553`
- action: 完成数据质量缺口下一轮复核建议摘要。
- status: `done`
- notes: 数据质量总览页现在展示“缺口下一轮复核建议”，把 owner/来源压力转成三步只读查看顺序，展示建议标题、首要 owner、首要来源、代表问题、影响异常、影响人员、查看入口和暂缓能力；模型基于现有本地缺口 owner/来源压力、代表问题、字段和人员只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US548-US550`
- action: 完成数据质量缺口 owner/来源压力摘要。
- status: `done`
- notes: 数据质量总览页现在展示“缺口 owner/来源压力”，按责任人与数据来源汇总未覆盖复核缺口，并展示缺口问题、影响异常、影响人员、首要 owner、首要来源、代表问题、字段、人员、查看入口和暂缓能力；模型基于现有本地复核覆盖缺口、owner、source、字段和人员只读聚合。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US545-US547`
- action: 完成数据质量复核覆盖缺口摘要。
- status: `done`
- notes: 数据质量总览页现在展示“复核覆盖缺口摘要”，对比当前复核路径和影响异常问题，识别未进入路径的问题、字段和人员，并展示首要缺口、查看入口、缺口原因、下一查看提示和暂缓能力；模型基于现有本地聚合结果只读推导。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- action: 完成数据质量复核路径顺序。
- status: `done`
- notes: 数据质量总览页现在展示“复核路径顺序”，把优先问题、字段、日期、人员和原因排成连续查看路径，并展示每个步骤的查看入口、路径理由、影响异常、影响人员、下一查看提示和暂缓能力；模型基于现有本地聚合结果只读推导。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
...
- story_ids:
  - `US539-US541`
- action: 完成数据质量复核优先级说明。
- status: `done`
- notes: 数据质量总览页现在展示“复核优先级说明”，把影响异常 Top、原因、人员、日期和字段摘要汇成主管可读的首要复核理由，并展示首要字段、首要日期、首要人员、代表问题、下一查看提示和暂缓能力；模型基于现有本地聚合结果只读推导。本次不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式；current queue 与 active tasks 已清空。
