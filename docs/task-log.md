...
- status: `done`
- notes: `/person-timeline` 现在作为履约日历入口，团队按职场+项目映射，小组按供应商映射；成员矩阵每人默认展示排班、登录、状态三条横向子轨，并保留进入个人单日三轨详情的链路。本次不新增依赖，不改后端、数据库、真实接口、权限、审批、导出、批量、自动排班或生产公式；current queue 与 active tasks 已清空。
