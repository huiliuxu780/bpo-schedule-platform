...
`US587/F403 -> US595/Q120` completed the first G001 import-center, master-data, and personnel-schedule production-prototype foundation. `US596/F409 -> US597/F411` completed demand forecast CSV import, forecast version change tracking, and forecast-vs-schedule alignment with shortage, overstaffed, balanced, and no-matching-schedule statuses. Current queue starts `US598/Q121` for demand forecast foundation QA. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
