...
Recommended next step is import-center exception-state consolidation or upload/API failure guidance. Keep apply write buttons, multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
