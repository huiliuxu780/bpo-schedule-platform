...
IM275 is active on `codex/im275-schedule-draft-validation-review`. Scope: add a shared schedule-plan draft validation and review panel on top of IM274's interval editor, covering required plan fields, 0.5h interval validity, time order, overlap/break warnings, zero-value warnings, total gap review, and submit blocking for hard errors. Qoder may work only from `docs/prompts/im275-qoder-task-packets.md`; Codex owns current-state updates, diff review, browser verification, final check, commit, and push. `project-progress/**` remains local Feishu tracker output and must not be included in product commits unless a future Feishu reporting task explicitly allows it.
...
Current recommendation is to complete IM275 before opening another product slice. Keep the work inside the schedule-plan draft editor; do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, or charge factors unless a new task is defined.
