...
IM274 is active on `codex/im274-schedule-plan-draft-editor-workbench`. Scope: upgrade `/schedule-plans/new` and `/schedule-plans/[planId]/edit` into a complete schedule-plan draft interval editor workbench with shared create/edit form, 0.5h interval rows, add/delete row controls, save-before-review summary, draft feedback, non-draft blockers, and browser acceptance. Qoder may work only from `docs/prompts/im274-qoder-task-packets.md`; Codex owns current-state updates, diff review, final verification, commit, and push. `project-progress/**` remains local Feishu tracker output and must not be included in product commits unless a future Feishu reporting task explicitly allows it.
...
Current recommendation is to complete IM274 before opening another product slice. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, or charge factors unless a new task is defined.
