...
`US750/IM130` through `US815/IM195` corrected visible product structure, fixed the recovery plan, extracted shared API/import utilities, added runtime guards, added error/loading recovery, split import/master-data giant files, codified action placement scopes, introduced shared empty/form/header rules, removed old planning demo entries, clarified version semantics, added Qoder-executed helper guards, and continued shared MetricCard cleanup across old schedule/unavailability pages. Current queue is empty; seed the next smallest confirmed slice before development.
