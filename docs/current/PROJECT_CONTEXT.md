...
`US750/IM130` through `US802/IM182` corrected visible product structure, fixed the recovery plan, extracted shared frontend API result/error and import fetch utilities, added runtime guards, added global error/loading recovery, split import/master-data giant files, and codified visible action placement scopes for page, filter, list, row, and danger actions. The current ready queue is empty; do not guess the next task before the next requirement is split and confirmed. Future slices must not change visible UI, navigation, backend, database, packages, permissions, approval, export, batch operation, automatic scheduling, formula, settlement, supplier-contract, minimum-staffing, charge-factor, or revive quality/import center navigation unless a confirmed Gate explicitly allows it.
