...
`US587/F403 -> US589/Q118` completed the first G001 import-center foundation: CSV upload and field-mapping preview across five import types, import batch business date/version records, failed-row traceability, backend/API tests, product copy checks, detail-page smoke, and final project check. Current queue now starts `US590/F405 -> US595/Q120` for master data and personnel schedule foundations; forecast and login/status waves remain blocked in backlog until dependencies pass. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
