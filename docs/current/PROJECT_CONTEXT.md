...
Current recommendation is to build the next roster slice around Forecast vs Arranged/Actual headcount gaps from the edited roster draft preview, using the existing local fixture/generator and derived coverage only. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, charge factors, automatic scheduling, database work, forecasting-model work, standard-capacity work, Excel upload/import, or real external integrations unless a new task is defined and confirmed.
