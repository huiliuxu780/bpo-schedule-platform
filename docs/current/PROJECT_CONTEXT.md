...
`US750/IM130` through `US834/IM214` corrected visible product structure, fixed the recovery plan, extracted shared API/import utilities, added runtime guards, added error/loading recovery, split import/master-data giant files, codified action placement scopes, introduced shared empty/form/header rules, removed old planning demo entries, clarified version semantics, added Qoder-executed helper guards, continued shared MetricCard/list-filter cleanup, migrated the first seven SimpleTable slices, defined and guarded the MainTableShell boundary, completed MainTableShell on `schedule-plan-table`/`unavailability-table`, closed the table-abstraction chain, confirmed dashboard anomaly `data-table` remains an overview widget, defined future dashboard downstream entries as summary-to-workspace links only, changed rows without stable downstream IDs to a blocked entry state, closed the dashboard anomaly chain, calibrated the 2026-06-14 project understanding document as product baseline rather than current backlog, and calibrated the review-case workspace as a real controlled local exception workflow whose approval/export/permission/batch/production semantics still require new Gates. Current queue is empty; seed the next smallest confirmed slice before development.
