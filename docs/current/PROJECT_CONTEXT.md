...
`US750/IM130` through `US836/IM216` corrected visible product structure, fixed recovery and shared UI/util seams, migrated safe table abstractions, defined MainTableShell boundaries, closed dashboard anomaly scope, calibrated the requirements baseline, calibrated the review-case workspace as a controlled local exception workflow, closed IM215 as no-new-env QA after live 3000 smoke was blocked by missing 8000 API, and hardened the import-center model test runner so review-case processing/action/continuation assertions run in `scripts/check.sh`. Current queue is empty; future live review-case acceptance still requires an approved 8000/API runtime.
