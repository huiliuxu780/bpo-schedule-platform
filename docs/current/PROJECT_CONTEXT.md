...
`US750/IM130` through `US828/IM208` corrected visible product structure, fixed the recovery plan, extracted shared API/import utilities, added runtime guards, added error/loading recovery, split import/master-data giant files, codified action placement scopes, introduced shared empty/form/header rules, removed old planning demo entries, clarified version semantics, added Qoder-executed helper guards, continued shared MetricCard/list-filter cleanup, migrated the first seven SimpleTable slices, defined and guarded the MainTableShell boundary, completed MainTableShell implementations on `schedule-plan-table` and `unavailability-table`, and closed the table-abstraction chain by deferring `data-table` until dashboard anomaly ownership is clear. Current queue is empty; seed the next smallest confirmed slice before development.
