...
`US750/IM130` through `US853/IM233` corrected visible product structure, fixed shared UI/util seams, migrated safe table abstractions, defined MainTableShell boundaries, closed dashboard anomaly scope, calibrated requirements/review-case QA, and split import-center, review-case, master-data maintenance, production model, product-structure, dashboard model, batch-detail, batch-apply, agent, reference, plus remaining import-center gates. Current queue is empty; future live review-case acceptance still requires an approved 8000/API runtime.
