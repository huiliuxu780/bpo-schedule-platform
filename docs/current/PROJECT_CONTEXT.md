...
`US587/F403 -> US592/Q119` completed the first G001 import-center and master-data foundation: CSV preview, import batch failure rows/version records, master-data CSV submit, process-memory imported binding records, local add/update, freeze/unfreeze, effective-date and binding reference checks, page/API smoke, and master-data QA closeout. Current queue now starts `US593/F407 -> US595/Q120` for personnel schedule foundations; forecast and login/status waves remain blocked until dependencies pass. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
