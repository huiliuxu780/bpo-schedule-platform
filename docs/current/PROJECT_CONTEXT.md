...
Current recommendation is to implement the roster foundation in this order after the IM282 product contract: ShiftType work-segment parsing and half-hour expansion; personnel-level monthly roster draft model; previous-month/week stable-shift copy into a draft with pending employees; then `draft -> published` plus derived shift counts, half-hour coverage, and Forecast vs Arranged/Actual headcount gaps. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, charge factors, automatic scheduling, backend/database work, forecasting-model work, standard-capacity work, or real external integrations unless a new task is defined and confirmed.
