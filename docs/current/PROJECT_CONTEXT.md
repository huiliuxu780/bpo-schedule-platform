...
`US750/IM130` through `US791/IM171` corrected visible product structure, completed master-data CRUD/detail links, and moved forecast, schedule, and login/status-log imports back into owning business pages as step-by-step Dialogs. `US792/IM172` completed the frontend health recovery plan and Product Design gate; current queue returned to empty. Future recovery slices must be seeded one IM at a time from `docs/frontend-health-recovery-plan.md`; approval, export, batch operation, permission, automatic scheduling, formula, settlement, supplier-contract, minimum-staffing, charge-factor, revived quality/import center navigation, and package/lockfile changes remain excluded unless separately confirmed.
