...
`US750/IM130` through `US813/IM193` corrected visible product structure, fixed the recovery plan, extracted shared API/import utilities, added runtime guards, added error/loading recovery, split import/master-data giant files, codified action placement scopes, introduced shared empty/form/header rules, removed old planning demo entries, stopped current schedule-plan pages from linking into old demo routes, clarified forecast/schedule/log version entry semantics, aligned comparison-run detail with business-version result review navigation, clarified version-workbench comparison actions as comparison-run operations, and added a Qoder-executed structure guard for shared lib helpers. The current ready queue is empty; do not guess the next task before the next requirement is split and confirmed.
