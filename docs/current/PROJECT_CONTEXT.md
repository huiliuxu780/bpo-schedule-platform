...
`US587/F403 -> US595/Q120` completed the first G001 import-center, master-data, and personnel-schedule production-prototype foundation. `US596/F409` completed demand forecast CSV import into process-memory forecast rows with skill/grade validation, failure-row blocking, source batch, and forecast version traceability. Current queue starts `US596/F410` for forecast version change tracking; keep `F411` forecast-vs-schedule alignment separate. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
