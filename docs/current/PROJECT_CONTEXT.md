...
Recommended next step after `IM022` is verified is to continue with one small production-usable import-center slice: extend the same apply-before-write readiness safety gate to personnel_schedule and login/status-log apply, or wire import-center APIs into the existing frontend. Keep multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
