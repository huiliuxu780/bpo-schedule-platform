...
`US750/IM130` through `US856/IM236` corrected visible product structure, fixed shared UI/util seams, migrated safe table abstractions, defined MainTableShell boundaries, closed dashboard anomaly scope, calibrated requirements/review-case QA, split import-center, review-case, version workbench, master-data maintenance, production model, product-structure, dashboard model, batch-detail, batch-apply, agent, reference, plus structure/master-data gates, and added operator-facing review-case processing paths without exposing Codex/Gate/PM acceptance language in product pages. Current queue is empty; future live review-case acceptance still requires an approved 8000/API runtime.
