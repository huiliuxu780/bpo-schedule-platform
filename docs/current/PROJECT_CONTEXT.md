...
Current recommendation is to prepare the roster persistence/publish product contract before any real API/database implementation: define draft storage, publish semantics, versioning, audit trail, and rollback boundaries separately from the frontend-local workbench. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, charge factors, automatic scheduling, database work, forecasting-model work, standard-capacity work, Excel upload/import, or real external integrations unless a new task is defined and confirmed.
