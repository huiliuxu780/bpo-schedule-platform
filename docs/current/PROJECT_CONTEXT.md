...
`US750/IM130` through `US793/IM173` corrected visible product structure, fixed the recovery plan, and extracted shared frontend `ApiResult<T>` / `formatApiError` utilities. Current queue returned to empty. Next recovery slice should be seeded from IM174 import fetch utility extraction; visible UI work still requires Product Design gate, and backend, database, packages, permissions, approval, export, batch operation, automatic scheduling, formula, settlement, supplier-contract, minimum-staffing, charge-factor, and revived quality/import center navigation remain excluded unless separately confirmed.
