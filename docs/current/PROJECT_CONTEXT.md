...
Recommended next step after `IM020` is verified is to continue with one small production-usable import-center slice: frontend import-center API wiring or a narrow readiness-detail extension. Keep multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
