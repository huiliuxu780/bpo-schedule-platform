...
`US750/IM130` corrected visible product structure: 经营总览不再挂载数据接入状态面板，侧边栏不再暴露占位 dashboard 链接或暂缓的权限、结算、接口集成、自动排班入口。`US751/IM131` completed the personnel master-data model base: organizations support hierarchy/path, employees support type plus organization/workplace links, skills support category, and employee multi-skill rows are persisted/importable。`US752/IM132` completed the real personnel list loop: `/api/v1/master-data/employees` returns employee rows with organization/workplace/skill context, and `/master-data/agents` is now a customer-service personnel management list. `US753/IM133` completed the single-agent core edit loop: create/edit forms submit employee type, organization ID, and workplace ID to the single-employee maintenance API. `US754/IM134` completed single-agent skill-set replace maintenance: create/edit/skill maintenance use separate personnel subpages, freeze uses a modal, and the list stays focused on table management. `US755/IM135` polished agent-list UI details. IM136 corrected business-first navigation, split non-agent master-data routes into list pages only, removed stale data-ingestion/CORN/task-code/operator-default/center-first copy residue, masked task-code batch labels in visible UI, and changed upload feedback away from center-first batch wording. IM137 keeps `/master-data/skills` readable against older local SQLite data and masks task-code/smoke identifiers. IM138 removes project as a master-data product object, adds `/master-data/organizations` as a read-only organization list, and hides the legacy project dimension from binding UI while keeping compatibility fields out of scope for deletion. IM139-IM140 clarifies workplace as a location-only master-data object; skills remain an independent master-data entry, while standalone workplace operating-owner and binding relationship entries are removed from navigation and entity routing pending object-detail designs. IM141 adds `/master-data/sites/[workplaceId]` as the workplace child detail page; IM142 adds `/master-data/vendors/[vendorId]` as the supplier child detail page with service-workplace context linked back to workplace detail. IM145 removes standalone `预测生产` and `排班生产` Sidebar entries while keeping their routes under `需求计划` and `排班计划` active ownership. IM146 keeps production child routes compatible but changes visible titles, list titles, return actions, and model guidance to `预测版本`、`排班版本` and `登录/状态日志` business wording. IM147 adds Breadcrumbs to demand, schedule, actual-log, and data-quality compatible pages and removes duplicate content-area page h1 headings. IM148 removes the old hidden Header searchPlaceholder API and page passthroughs while preserving real list filters in content areas. IM149 removes unconfirmed non-agent master-data content import actions while keeping confirmed agent actions in Header. IM150 removes the generic CSV upload action from the import-batch ledger and moves forecast, schedule, and actual-log import entries into business page Header actions. IM151 removes import-batch parent Breadcrumbs from result-chain pages while preserving compatibility routes. IM152 changes visible master-data workplace wording from operating subject to service team. IM153 removes global form font override drift and aligns Button, Table, agent row actions, and agent import Dialog typography to the 14px B-end baseline. IM154 adds the workplace CRUD frontend loop: `/master-data/sites` Header create action, row detail/edit/freeze actions, create/edit child pages, and freeze Dialog backed by the existing workplace reference maintenance API. IM155 adds the supplier CRUD frontend loop with the same child-page/Dialog pattern for `/master-data/vendors`. IM156 adds the skill CRUD loop with `/master-data/skills/new`, `/master-data/skills/[skillId]/edit`, freeze Dialog, and true `skill_category` maintenance. IM157 adds the organization CRUD loop with `/master-data/organizations/new`, `/master-data/organizations/[organizationId]/edit`, freeze Dialog, and a narrow local organization maintenance API without schema/migration changes. IM158 makes `/master-data/agents` skill, organization, and workplace filters derive from real personnel rows and participate in URL filtering. IM159 adds SQLite-only local schema compatibility so older local master-data tables gain confirmed employee, skill, and organization structure before maintenance writes. IM160 makes workplace detail service teams read-only and grouped by internal organization teams plus supplier-bound teams. IM161 adds the workplace-detail-scoped service-team maintenance loop with local API/table, nested create/edit pages, and freeze Dialog. IM162 adds the nested workplace-service-team detail page. IM163 adds the same detail page's read-only associated-person list using existing employee and binding data. IM164 adds supplier-detail read-only service-team links back to existing workplace service-team details. IM165 adds a read-only customer-service personnel detail page with service-team links. IM166 adds a read-only organization detail page with direct child organizations and personnel links. IM167 adds a read-only skill detail page with personnel links. IM168 completes master-data detail-link consistency. IM169 completes the demand forecast import Dialog on `/demand-plans/production`; IM170 completes the schedule import Dialog on `/schedule-plans/production`; IM171 completes login/status-log import Dialogs on `/actual-logs/production`; current queue returns to empty. Approval, export, batch operation, permission, automatic scheduling, formula, settlement, supplier-contract, minimum-staffing, and charge-factor scope remain excluded.
