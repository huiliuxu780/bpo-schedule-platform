...
`US750/IM130` through `US803/IM183` corrected visible product structure, fixed the recovery plan, extracted shared API/import utilities, added runtime guards, added error/loading recovery, split import/master-data giant files, codified action placement scopes, and introduced the first shared empty-state component. The current ready queue is empty; do not guess the next task before the next requirement is split and confirmed.
