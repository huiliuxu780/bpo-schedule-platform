...
Recommended next step after `US668/IM048` is data-quality-to-exception reverse aggregation drilldown: show which import quality issues affect the most downstream exceptions. Keep apply write buttons, multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
