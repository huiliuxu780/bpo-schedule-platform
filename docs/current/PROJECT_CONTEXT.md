...
Recommended next step after `IM019` is verified is to continue with one small production-usable import-center slice: apply-readiness validation or a frontend import-center API wiring slice. Keep multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
