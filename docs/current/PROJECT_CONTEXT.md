...
Recommended next step after `IM021` is verified is to continue with one small production-usable import-center slice: frontend import-center API wiring or a narrow import apply safety extension. Keep multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
