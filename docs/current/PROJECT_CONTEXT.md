...
Recommended next step after `IM018` is verified is to continue with one small production-usable import-center slice: mapping-template update/deactivate controls or apply-readiness validation. Keep multipart/Excel, external integrations, auth, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement rules, and charge factors out of scope until separate tasks.
