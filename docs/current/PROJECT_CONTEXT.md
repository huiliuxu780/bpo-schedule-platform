...
Current recommendation is to build the next roster slice around `draft -> published` state expression and derived shift counts/half-hour coverage from the edited draft preview, then connect Forecast vs Arranged/Actual headcount gaps after the workbench is stable. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, charge factors, automatic scheduling, database work, forecasting-model work, standard-capacity work, Excel upload/import, or real external integrations unless a new task is defined and confirmed.
