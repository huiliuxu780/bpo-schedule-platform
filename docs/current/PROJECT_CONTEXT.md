...
`US587/F403 -> US588/F404` completed the first G001 import-center implementation slice: the import page accepts real CSV files across five import types, previews field mappings before import, and import batches now expose business date ranges plus generated import version records for successful rows while preserving failed row traceability. Current queue now starts `US589/Q118 -> US595/Q120`; forecast and login/status waves remain blocked in backlog until dependencies pass. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
