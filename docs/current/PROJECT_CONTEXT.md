...
Current recommendation is to continue the roster foundation after IM285 in this order: harden generated draft editing/adjustment in the visible roster workbench; then add `draft -> published` and derived shift counts/half-hour coverage; then connect Forecast vs Arranged/Actual headcount gaps after the generation surface is stable. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, charge factors, automatic scheduling, database work, forecasting-model work, standard-capacity work, Excel upload/import, or real external integrations unless a new task is defined and confirmed.
