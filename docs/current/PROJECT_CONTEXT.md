...
`US587/F403 -> US601/F414` completed the G001 import-center, master-data, personnel-schedule, demand-forecast, login-log, and status-log foundation through local CSV import, versions, login business-day normalization, status dictionary validation, and actual-log 0.5h slicing with gap/overlap quality issues. Current queue starts `US602/F415` for schedule-vs-actual anomaly generation. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
