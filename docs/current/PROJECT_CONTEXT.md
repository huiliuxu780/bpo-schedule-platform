...
current_summary:
  queue_state: active
  active_batch_id: BATCH-PLAN-LIST-RISK-ENTRY-001
  in_progress_task: F102
  ready_tasks: [F103, Q032]
```
...
The active product batch is `US139 / F102-F103 / Q032` on branch `codex/f073-review-checklist-h032`. It closes the remaining schedule-plan risk-entry context gap: when a user enters the risk workbench from the schedule-plans risk summary card or the embedded risk preview table, the destination and continuation actions must preserve `schedule-plans-list`, `query`, and `status` instead of dropping into a generic risk route.
...
Continue `US139 / F102-F103 / Q032` on branch `codex/f073-review-checklist-h032`. Keep the same no-database schedule-plan review chain coherent by preserving plan-list context for the schedule-plans risk summary entry and embedded risk preview row actions.
