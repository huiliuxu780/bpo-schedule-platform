...
No current story or active task is queued after IM275 completion. Define the next product slice before opening implementation. `project-progress/**` remains local Feishu tracker output and must not be included in product commits unless a future Feishu reporting task explicitly allows it.
...
Current recommendation is to define the next medium product slice before implementation. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, or charge factors unless a new task is defined and confirmed.
