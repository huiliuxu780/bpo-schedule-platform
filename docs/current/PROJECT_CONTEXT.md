...
`US750/IM130` through `US796/IM176` corrected visible product structure, fixed the recovery plan, extracted shared frontend API result/error and import fetch utilities, added runtime guards for data-quality Server Action inputs, and added the global `app/error.tsx` recovery boundary. The current ready queue is empty; do not guess the next task before the next requirement is split and confirmed. Future slices must not change visible UI, navigation, backend, database, packages, permissions, approval, export, batch operation, automatic scheduling, formula, settlement, supplier-contract, minimum-staffing, charge-factor, or revive quality/import center navigation unless a confirmed Gate explicitly allows it.
