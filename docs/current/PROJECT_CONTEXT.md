...
After that empty-state checkpoint, the next chain is reseeded for demand-forecast production. Only `US722/IM102` is current-ready: add `/demand-plans/production` as a read-only workbench under the existing plan/schedule area, showing forecast versions, source batches, application status, skill-group/level/time-bucket alignment state, and blockers. `US723/IM103` and `US724/IM104` remain outside current state until the prior slice is green. Keep avoiding real write APIs, backend/schema/migration, permissions, approval, export, batch operations, automatic scheduling, production formulas, settlement, and charge-factor work unless a new confirmed task allows it.
