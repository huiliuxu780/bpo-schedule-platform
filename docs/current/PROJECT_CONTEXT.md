...
No executable story is currently queued. IM270 restored `/dashboard` and the global shell against the shadcn dashboard-01 structure after PM rejected earlier piecemeal styling fixes: the sidebar now uses the dashboard-01 offcanvas frame, master-data entries have distinct icons, KPI cards use `CardAction`/`CardFooter` with dashboard-01 metric-card rhythm, the main trend chart is full-width through the shadcn chart container, and the table workbench follows the reference tabs/column-control spacing. IM268/IM269 boundaries still apply: no placeholder sidebar actions, disabled logout, fake chart range controls, implementation-facing data-source banners, or top-level dashboard filter console above KPI cards.
...
- IM270 restored the dashboard-01 structure after PM rejected the remaining fake-alignment work. `AppSidebar` was rebuilt as an offcanvas dashboard-01-style sidebar with flat workbench navigation and distinct master-data icons; `components/ui/card.tsx` now exposes the missing `CardAction` slot; KPI cards use `CardAction`/`CardFooter`, shadcn metric-card grid/shadow rhythm, and full-card drilldowns; `/dashboard` removes the cramped side heatmap from the first screen and gives the trend chart a full-width shadcn `ChartContainer`; the anomaly table keeps its real filters but follows the reference tabs/column-control layout. The heatmap model remains available for downstream use, but it is no longer rendered in the dashboard first screen.
