...
`US587/F403 -> US593/F407` completed the first G001 import-center, master-data, and personnel-schedule import foundation: CSV preview, import batch failure rows/version records, master-data maintenance/reference checks, personnel schedule imported records, shift-type reference validation, schedule version/source batch traceability, and page/API smoke. Current queue now starts `US594/F408 -> US595/Q120` for 0.5h personnel-schedule expansion and QA; forecast and login/status waves remain blocked until dependencies pass. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
