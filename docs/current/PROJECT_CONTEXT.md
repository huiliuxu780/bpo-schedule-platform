...
`US750/IM130` through `US835/IM215` corrected visible product structure, fixed the recovery plan, extracted shared API/import utilities, added runtime guards, added error/loading recovery, split import/master-data giant files, codified action placement scopes, introduced shared empty/form/header rules, removed old planning demo entries, clarified version semantics, added Qoder-executed helper guards, continued shared MetricCard/list-filter cleanup, migrated the first seven SimpleTable slices, defined and guarded the MainTableShell boundary, completed MainTableShell on `schedule-plan-table`/`unavailability-table`, closed the table-abstraction chain, confirmed dashboard anomaly `data-table` remains an overview widget, closed the dashboard anomaly chain, calibrated the 2026-06-14 project understanding document, calibrated the review-case workspace as a controlled local exception workflow, and blocked live review-case acceptance smoke because 3000 pages are reachable but the required 8000 API is not running. Current queue contains blocked `US835/IM215`; unblock by allowing an approved backend/API runtime, providing a running API, or downgrading to model/contract-only acceptance.
