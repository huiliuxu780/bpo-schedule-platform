...
`US750/IM130` through `US801/IM181` corrected visible product structure, fixed the recovery plan, extracted shared frontend API result/error and import fetch utilities, added runtime guards for data-quality Server Action inputs, added the global `app/error.tsx` recovery boundary, added route-local loading skeletons for the core business entries, split the import-center model into responsibility files, split the master-data maintenance workbench into UI responsibility files, and split the master-data maintenance model into types, entities, payloads, agents, references, details, import-dialog, and formatter responsibility files. The current ready queue is empty; do not guess the next task before the next requirement is split and confirmed. Future slices must not change visible UI, navigation, backend, database, packages, permissions, approval, export, batch operation, automatic scheduling, formula, settlement, supplier-contract, minimum-staffing, charge-factor, or revive quality/import center navigation unless a confirmed Gate explicitly allows it.
