...
`US587/F403 -> US595/Q120` completed the first G001 import-center, master-data, and personnel-schedule production-prototype foundation. `US596/F409 -> F410` completed demand forecast CSV import into process-memory forecast rows plus forecast version change tracking with previous/new source batch, version, and forecast-agent deltas. Current queue starts `US597/F411` for forecast-vs-schedule alignment. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
