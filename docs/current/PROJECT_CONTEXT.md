...
Current recommendation is to keep the next roster slice on scheduler-visible draft hardening: either strengthen gap-row drilldown/actions inside the existing local workbench, or prepare the future persistence contract separately before any real publish/database work. Do not add another dashboard top-level filter console, claim production readiness, install E2E dependencies, or expand into permissions, approval, export, batch operations, formulas, settlement rules, charge factors, automatic scheduling, database work, forecasting-model work, standard-capacity work, Excel upload/import, or real external integrations unless a new task is defined and confirmed.
