...
`US587/F403 -> US594/F408` completed the first G001 import-center, master-data, and personnel-schedule production-prototype foundation: CSV preview, import batch failure rows/version records, master-data maintenance/reference checks, personnel schedule imported records, shift-type reference validation, schedule version/source batch traceability, 0.5h interval expansion, schedule-detail/person trace links, and page/API smoke. Current queue now starts `US595/Q120` for personnel-schedule foundation QA; forecast and login/status waves remain blocked until dependencies pass. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
