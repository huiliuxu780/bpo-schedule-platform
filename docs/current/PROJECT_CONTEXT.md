...
Current story queue and active task list are empty after IM273. Do not guess the next product task; define the next user-visible capability before implementation. `project-progress/**` remains local Feishu tracker output and must not be included in product commits unless a future Feishu reporting task explicitly allows it.
