...
`US587/F403` completed the G001 CSV upload and field-mapping preview slice: the import page now accepts real CSV files across master data, personnel schedule, demand forecast, login log, and status log types, then previews detected fields, row counts, missing required fields, and pending validations before import. Current queue now starts `US588/F404 -> US595/Q120`; forecast and login/status waves remain blocked in backlog until dependencies pass. Continue to block database, ORM, migration, dependency, external integration, auth, permission, approval, export, batch, file storage, Excel xlsx parsing, automatic scheduling, settlement, charge-factor, and production formula work.
