...
requirements:
  - id: R602
    module: "数据质量"
    description: "现场主管需要在数据质量总览看到分组步骤 owner 交接摘要，以便把复核队列转成给责任人的交接口径。"
  - id: R603
    module: "数据质量"
    description: "owner 交接摘要需要基于现有本地分组步骤 owner 复核队列聚合，不新增后端接口、数据库持久化或生产处理状态。"
  - id: R604
    module: "数据质量"
    description: "owner 交接摘要需要展示 owner、代表问题、代表人员、关联分组、交接要点、质量问题入口、人员履约入口、查看提示和暂缓能力。"
  - id: R605
    module: "质量与交付"
    description: "数据质量分组步骤 owner 交接摘要完成后需要 QA 收口，确认模型测试、页面展示、页面 smoke 和 no-action 边界。"
source: "PM requested continuing after F396 on 2026-05-26"
submitted_at: "2026-05-26"
version: "1.0"
status: "split"
notes: "本组只做数据质量总览页的本地只读分组步骤 owner 交接摘要，基于已有 owner 复核队列聚合。不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式。"
```

### R598-R601 - 数据质量分组步骤 owner 复核队列

```yaml
requirements:
  - id: R598
    module: "数据质量"
    description: "现场主管需要在数据质量总览看到分组步骤 owner 复核队列，以便把 owner/人员负载转成下一步只读查看顺序。"
  - id: R599
    module: "数据质量"
    description: "owner 复核队列需要基于现有本地分组步骤 owner/人员负载聚合，不新增后端接口、数据库持久化或生产处理状态。"
  - id: R600
    module: "数据质量"
    description: "owner 复核队列需要展示 rank、owner、代表问题、代表人员、关联分组、队列理由、质量问题入口、人员履约入口、查看提示和暂缓能力。"
  - id: R601
    module: "质量与交付"
    description: "数据质量分组步骤 owner 复核队列完成后需要 QA 收口，确认模型测试、页面展示、页面 smoke 和 no-action 边界。"
source: "PM requested continuing after F395 on 2026-05-26"
submitted_at: "2026-05-26"
version: "1.0"
status: "split"
notes: "本组只做数据质量总览页的本地只读分组步骤 owner 复核队列，基于已有 owner/人员负载聚合。不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式。"
```

### R594-R597 - 数据质量分组步骤 owner/人员负载摘要

```yaml
requirements:
  - id: R594
    module: "数据质量"
    description: "现场主管需要在数据质量总览看到分组步骤按 owner 和人员汇总的负载摘要，以便判断先协调哪个责任人。"
  - id: R595
    module: "数据质量"
    description: "owner/人员负载摘要需要基于现有本地分组步骤影响对象聚合，不新增后端接口、数据库持久化或生产处理状态。"
  - id: R596
    module: "数据质量"
    description: "owner/人员负载摘要需要展示 owner、步骤数、影响人员、代表问题、质量问题入口、人员履约入口、查看提示和暂缓能力。"
  - id: R597
    module: "质量与交付"
    description: "数据质量分组步骤 owner/人员负载摘要完成后需要 QA 收口，确认模型测试、页面展示、页面 smoke 和 no-action 边界。"
source: "PM requested continuing after F394 on 2026-05-26"
submitted_at: "2026-05-26"
version: "1.0"
status: "split"
notes: "本组只做数据质量总览页的本地只读分组步骤 owner/人员负载摘要，基于已有分组步骤影响对象聚合。不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式。"
```
...
requirements:
  - id: R586
    module: "数据质量"
    description: "现场主管需要在数据质量总览看到质量分组复核顺序，以便按影响履约异常的程度决定先看哪个分组。"
  - id: R587
    module: "数据质量"
    description: "分组复核顺序需要基于现有本地质量分组异常影响覆盖聚合，不新增后端接口、数据库持久化或生产处理状态。"
  - id: R588
    module: "数据质量"
    description: "分组复核顺序需要展示步骤、owner、风险、代表问题、影响异常、影响人员、阻断行、分组入口、查看提示和暂缓能力。"
  - id: R589
    module: "质量与交付"
    description: "数据质量分组复核顺序完成后需要 QA 收口，确认模型测试、页面展示、页面 smoke 和 no-action 边界。"
source: "PM requested continuing after F392 on 2026-05-26"
submitted_at: "2026-05-26"
version: "1.0"
status: "split"
notes: "本组只做数据质量总览页的本地只读分组复核顺序摘要，基于已有 fallback 质量分组、数据质量问题和分组异常影响覆盖聚合。不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式。"
```
...
requirements:
  - id: R502
    module: "导入批次"
    description: "现场主管需要在导入批次详情顶部看到失败原因汇总，快速判断主要失败字段、错误码和失败行数。"
  - id: R503
    module: "导入批次"
    description: "失败原因汇总需要基于现有失败行明细本地聚合，不新增数据库、后端持久化或生产处理状态。"
  - id: R504
    module: "导入批次"
    description: "失败原因汇总需要展示影响对象、首要修正提示和代表行，帮助现场主管先定位修正顺序。"
  - id: R505
    module: "质量与交付"
    description: "导入失败原因汇总完成后需要 QA 收口，确认模型测试、页面展示、浏览器 smoke 和 no-action 边界。"
source: "PM agreed to continue with import failure reason summary on 2026-05-26"
submitted_at: "2026-05-26"
version: "1.0"
status: "split"
notes: "本组只做导入批次详情页的本地只读失败原因汇总，基于已有 failureRows 和 affectedObjects 聚合。不新增后端接口、依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、生产状态字典、自动排班、结算、收费因子或生产公式。"
```
...
requirements:
  - id: R494
    module: "导入批次"
    description: "现场主管需要上传状态日志 CSV 文件，并获得批次编号、解析结果、成功行、失败行和错误码。"
  - id: R495
    module: "导入批次"
    description: "状态日志 CSV 导入需要校验状态日志 ID、员工、业务日期、状态类型、开始时间、结束时间、职场、项目和来源系统等必填字段，并保留失败行号、字段名、原值和错误信息。"
  - id: R496
    module: "导入批次"
    description: "导入批次页面需要提供状态日志 CSV 上传入口，提交后进入批次详情查看解析汇总和失败行明细。"
  - id: R497
    module: "质量与交付"
    description: "状态日志 CSV 导入纵切完成后需要 QA 收口，确认本地上传、标准库解析、失败行记录、浏览器 smoke 和 no-database 边界。"
source: "PM requested directly splitting and finishing the next upload/import tasks on 2026-05-26"
submitted_at: "2026-05-26"
version: "1.0"
status: "split"
notes: "本组只做状态日志 CSV 的本地 FastAPI process-memory 纵切：前端文件选择、server action 读取文本、后端标准库 CSV 解析、批次结果和失败行明细。不新增依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、自动排班、生产状态字典、结算、收费因子或生产公式。"
```

### R490-R493 - 登录日志 CSV 本地导入纵切

```yaml
requirements:
  - id: R490
    module: "导入批次"
    description: "现场主管需要上传登录日志 CSV 文件，并获得批次编号、解析结果、成功行、失败行和错误码。"
  - id: R491
    module: "导入批次"
    description: "登录日志 CSV 导入需要校验登录日志 ID、员工、业务日期、登录时间、登出时间、职场、项目和来源系统等必填字段，并保留失败行号、字段名、原值和错误信息。"
  - id: R492
    module: "导入批次"
    description: "导入批次页面需要提供登录日志 CSV 上传入口，提交后进入批次详情查看解析汇总和失败行明细。"
  - id: R493
    module: "质量与交付"
    description: "登录日志 CSV 导入纵切完成后需要 QA 收口，确认本地上传、标准库解析、失败行记录、浏览器 smoke 和 no-database 边界。"
source: "PM requested continuing the real upload/import Gate on 2026-05-25"
submitted_at: "2026-05-25"
version: "1.0"
status: "split"
notes: "本组只做登录日志 CSV 的本地 FastAPI process-memory 纵切：前端文件选择、server action 读取文本、后端标准库 CSV 解析、批次结果和失败行明细。不新增依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、自动排班或生产公式。"
```

### R486-R489 - 人员级排班 CSV 本地导入纵切

```yaml
requirements:
  - id: R486
    module: "导入批次"
    description: "排班运营需要上传人员级排班 CSV 文件，并获得批次编号、解析结果、成功行、失败行和错误码。"
  - id: R487
    module: "导入批次"
    description: "人员级排班 CSV 导入需要校验排班明细 ID、版本、员工、业务日期、职场、供应商、项目、班次、开始、结束和状态等必填字段，并保留失败行号、字段名、原值和错误信息。"
  - id: R488
    module: "导入批次"
    description: "导入批次页面需要提供人员级排班 CSV 上传入口，提交后进入批次详情查看解析汇总和失败行明细。"
  - id: R489
    module: "质量与交付"
    description: "人员级排班 CSV 导入纵切完成后需要 QA 收口，确认本地上传、标准库解析、失败行记录、浏览器 smoke 和 no-database 边界。"
source: "PM requested continuing the real upload/import Gate on 2026-05-25"
submitted_at: "2026-05-25"
version: "1.0"
status: "split"
notes: "本组只做人员级排班 CSV 的本地 FastAPI process-memory 纵切：前端文件选择、server action 读取文本、后端标准库 CSV 解析、批次结果和失败行明细。不新增依赖、数据库、ORM、migration、真实外部接口、权限、审批、导出、批量、Excel xlsx 解析、自动排班或生产公式。"
```
...
requirements:
  - id: R462
    module: "履约日历"
    description: "小组周视图需要展示周度责任压力，帮助现场主管识别本周哪些负责角色承接最多异常和升级压力。"
  - id: R463
    module: "履约日历"
    description: "周度责任压力需要包含负责角色、异常数、高优数、升级数、阻塞证据数、影响人员、影响日期、影响时长和下钻建议。"
  - id: R464
    module: "履约日历"
    description: "周度责任压力只能表达查看和解释口径，不提供真实派单、提交、保存、审批、导出、批量、真实处理或状态写入能力。"
  - id: R465
    module: "质量与交付"
    description: "周度责任压力完成后需要 QA 收口，确认模型测试、页面顺序、浏览器 smoke、文案边界和 no-action 边界。"
source: "PM requested continued supervisor-side production MVP development on 2026-05-25"
submitted_at: "2026-05-25"
version: "1.0"
status: "split"
notes: "本组只在现有履约日历小组周视图补充本地前端只读周度责任压力；不新增页面、导航、依赖、后端、数据库、真实接口、权限、审批、导出、批量、自动排班或生产公式。"
```
...
requirements:
  - id: R458
    module: "履约日历"
    description: "小组周视图需要展示周度质量影响汇总，帮助现场主管识别本周哪些数据质量问题正在影响异常复核。"
  - id: R459
    module: "履约日历"
    description: "周度质量影响汇总需要包含影响异常数、影响人员、影响日期、影响时长、严重度、阻塞证据、下钻路径和业务原因。"
  - id: R460
    module: "履约日历"
    description: "周度质量影响汇总只能表达查看和解释口径，不提供真实修复、提交、保存、审批、导出、批量、真实处理或状态写入能力。"
  - id: R461
    module: "质量与交付"
    description: "周度质量影响汇总完成后需要 QA 收口，确认模型测试、页面顺序、浏览器 smoke、文案边界和 no-action 边界。"
source: "PM requested continued supervisor-side production MVP development on 2026-05-25"
submitted_at: "2026-05-25"
version: "1.0"
status: "split"
notes: "本组只在现有履约日历小组周视图补充本地前端只读周度质量影响汇总；不新增页面、导航、依赖、后端、数据库、真实接口、权限、审批、导出、批量、自动排班或生产公式。"
```
...
requirements:
  - id: R406
    module: "履约日历"
    description: "团队下钻后的小组周视图需要展示周度主管交接摘要，帮助现场主管在本周复核队列之外看到需要向谁交接、仍有多少开放问题和升级关注。"
  - id: R407
    module: "履约日历"
    description: "周度主管交接摘要需要包含交接项数、开放问题数、升级项数、主要交接对象、下一查看项、交接触点和对应小组日期下钻入口。"
  - id: R408
    module: "履约日历"
    description: "周度主管交接摘要只能表达查看和交接准备口径，不提供通知、派单、提交、保存、审批、导出、批量、自动排班或状态写入能力。"
  - id: R409
    module: "质量与交付"
    description: "周度主管交接摘要完成后需要 QA 收口，确认模型测试、页面 smoke、文案审计和 no-action 边界。"
source: "PM requested continued supervisor handling workflow development on 2026-05-25"
submitted_at: "2026-05-25"
version: "1.0"
status: "split"
notes: "本组只在现有履约日历小组周视图补充本地前端只读交接摘要；不新增页面、导航、依赖、后端、数据库、真实接口、权限、审批、导出、批量、自动排班或生产公式。"
```
...
requirements:
  - id: R398
    module: "履约日历"
    description: "团队下钻后的小组周视图需要展示主管本周复核队列，帮助现场主管按小组和日期判断本周先看哪些履约问题。"
  - id: R399
    module: "履约日历"
    description: "主管本周复核队列需要包含队列摘要、最高优先级项、小组、日期、优先级、缺口人数、异常人数、建议先看对象和复核原因。"
  - id: R400
    module: "履约日历"
    description: "主管本周复核队列只能表达查看顺序和下钻线索，不提供通知、派单、提交、保存、审批、导出、批量、自动排班或状态写入能力。"
  - id: R401
    module: "质量与交付"
    description: "主管本周复核队列完成后需要 QA 收口，确认模型测试、页面 smoke、文案审计和 no-action 边界。"
source: "PM requested continued supervisor handling workflow development on 2026-05-25"
submitted_at: "2026-05-25"
version: "1.0"
status: "split"
notes: "本组只在现有履约日历团队下钻后的小组周视图补充本地前端只读队列；不新增页面、导航、依赖、后端、数据库、真实接口、权限、审批、导出、批量、自动排班或生产公式。"
```
...
requirements:
  - id: R362
    module: "履约日历"
    description: "异常详情需要展示主管跟进时间线，帮助现场主管知道异常已经跟进到哪里、当前卡点是什么。"
  - id: R363
    module: "履约日历"
    description: "异常队列模型需要返回识别、已跟进、当前卡点和下一复核节点。"
  - id: R364
    module: "履约日历"
    description: "主管跟进时间线仅作为查看口径，不提供通知、提交、保存、审批、导出、批量或状态写入能力。"
  - id: R365
    module: "质量与交付"
    description: "主管跟进时间线完成后需要 QA 收口，确认模型测试、页面 smoke、文案审计和 no-action 边界。"
source: "PM requested continued supervisor-oriented business completion on 2026-05-22"
submitted_at: "2026-05-22"
version: "1.0"
status: "split"
