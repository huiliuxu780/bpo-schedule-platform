...
version: "1.0"
status: "draft"
notes: "本需求用于防止遗漏真实排班业务：排班师主角色、系统内月班表生成、Excel 初始化边界、小组长/一线员工后续需求、预测模型未定、标准人力延后、自动排班延后都必须被追踪。当前只授权文档/建模，不授权开发、数据库、Excel 上传解析、外部集成、审批、权限、通知、导出、批量、生产公式、结算或收费因子。"
```
...
id: R948
module: "BPO WFM 三条主线"
description: "在 IM276 班组长排班板标准人力草稿闭环基础上，班组长需要能手动调整人员班次、是否纳入目标时段和计划技能，并即时看到标准人力覆盖、缺口、覆盖人数、技能错配和低能力补位变化。该能力只覆盖前端本地草稿交互，不做自动排班、真实保存、发布门槛或外部系统集成。"
source: "PM instruction on 2026-07-01: 直接开始; follows IM276 recommended next stage."
submitted_at: "2026-07-01"
version: "1.0"
status: "done"
notes: "IM277 已完成：`/schedule-plans/new` 班组长排班板支持本地手动调整草稿场景，点击移出 tay、加入 alex、加入 lily 后即时重算已排标准人力、缺口、覆盖人数、技能错配和满足状态。未新增自动排班、真实保存、发布门槛、审批、权限、导出、批量、外部集成、生产公式、结算或收费因子。"
```
...
id: R949
module: "BPO WFM"
description: "IM276/IM277 已证明单一时段标准人力覆盖和手动调整即时重算，但班组长还需要看到多个半小时时段的缺口队列，按缺口优先级选择要复核的时段。需要在不新增自动排班或真实保存的前提下，展示 10:00-10:30、10:30-11:00、11:00-11:30 等时段的需求、已排标准人力、缺口和原因，并支持点击队列项同步主明细。"
source: "PM confirmed IM278 after IM277 Done Report recommendation."
submitted_at: "2026-07-01"
version: "1.0"
status: "done"
notes: "IM278 已完成：`/schedule-plans/new` 的班组长排班板新增多半小时时段缺口队列，按标准人力缺口排序；保留 10:00-10:30 三人覆盖但 2.2 标准人力仍不满足样例，新增 10:30-11:00 缺口 1.0 优先补位和 11:00-11:30 已满足样例；点击队列项可同步主明细。未新增后端、数据库、依赖、package/lockfile、权限、审批、导出、批量、自动排班、生产公式、结算或收费因子。"
```
