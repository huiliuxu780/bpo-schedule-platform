...
if (traceLineCount > traceIndexStrictBudget) {
  ...
  warn(`TRACE_INDEX.yaml warning budget exceeded (${traceLineCount}/${traceIndexWarningBudget})`);
} else {
...
if (warnings > 0) {
  console.log(`check-state completed with ${warnings} warning(s) in ${mode} mode (diff=${diffMode}).`);
  if (strictLike) {
