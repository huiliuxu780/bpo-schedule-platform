...
export function generateStaticParams() {
  return productionMvpAlignmentReadinessSteps.map((step) => ({
    stepId: step.id,
  }))
}

export default async function ProductionMvpAlignmentReadinessStepPage({
  params,
}: PageProps) {
  const { stepId } = await params
  const step = getProductionMvpAlignmentReadinessStep(decodeURIComponent(stepId))
  ...
  return (
    <AppShell title={step.title} searchPlaceholder="搜索输入、输出或依赖">
      <main className="flex flex-1 flex-col gap-4 overflow-x-hidden overflow-y-auto p-4 lg:p-6">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <Metric title="输入对象" value={`${step.inputObjects.length}`} />
          <Metric title="输出物" value={`${step.outputArtifacts.length}`} />
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          ...
          <TagCard
            ...
            routePrefix="/production-mvp/alignment-readiness"
            emptyText="无"
          />
          <TagCard
            title="关联缺口"
