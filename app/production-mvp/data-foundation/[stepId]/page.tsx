...
export function generateStaticParams() {
  return productionMvpDataFoundationSteps.map((step) => ({
    stepId: step.id,
  }))
}

export default async function ProductionMvpDataFoundationStepPage({
  params,
}: PageProps) {
  const { stepId } = await params
  const step = getProductionMvpDataFoundationStep(decodeURIComponent(stepId))
      ...
      <main className="flex flex-1 flex-col gap-4 overflow-x-hidden overflow-y-auto p-4 lg:p-6">
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          ...
          <TagCard
            ...
            routePrefix="/production-mvp/data-foundation"
            emptyText="无"
          />
          <TagCard
            title="关联缺口"
